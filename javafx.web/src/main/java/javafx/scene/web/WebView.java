...
import com.sun.javafx.webkit.WebViewScreenSync;
...
final public class WebView extends Parent {
    ...
    public WebView() {
        ...
        // Keep the off-screen engine window's origin aligned with this node's
        // on-screen position so Blink's native popups (select/colour drop-downs)
        // appear over the control rather than at the engine window's origin.
        new WebViewScreenSync(this, page).install();
        stagePulseListener = () -> {
            handleStagePulse();
        };
    ...
    private void processMouseEvent(MouseEvent ev) {
        ...
        // Any press on the page dismisses an open context menu (it re-opens on a
        // fresh right-click). The WebView consumes mouse events, so the popup's own
        // auto-hide can't see the press — dismiss explicitly.
        if (ev.getEventType() == MouseEvent.MOUSE_PRESSED) {
            engine.hideContextMenu();
        }

        // Clicking the view must focus it (like any control) so the engine's
        // WebContents becomes focused and routes activation to the clicked
        // element. Without this, content clicks are ignored until the WebView
        // is focused another way (e.g. Tab traversal). Request focus on press,
        // before the event is forwarded, so the resulting FOCUS_GAINED command
        // precedes the mouse-down in the engine command ring.
        if (ev.getEventType() == MouseEvent.MOUSE_PRESSED && !isFocused()) {
            requestFocus();
        }
        ...
        // While an OSR popup (native <select>/colour drop-down) is open, route
        // ALL mouse events to it (popup-local coords): events inside select/hover,
        // events outside dismiss it — matching a native popup's mouse grab. The
        // page only sees mouse input again once the popup closes.
        double[] popupRect = page.getPopupRect();
        if (popupRect != null) {
            page.dispatchPopupMouseEvent(mouseEvent, popupRect[0], popupRect[1]);
            ev.consume();
            return;
        }
    ...
    private void processScrollEvent(ScrollEvent ev) {
        ...
        // Scrolling the page dismisses an open context menu.
        engine.hideContextMenu();
        ...
        // While an OSR popup is open, scroll IT (a long <select>/datalist list),
        // not the page — matching a native popup's input grab, like the mouse
        // routing in processMouseEvent.
        double[] popupRect = page.getPopupRect();
        if (popupRect != null) {
            page.dispatchPopupWheelEvent(wheelEvent, popupRect[0], popupRect[1]);
            ev.consume();
            return;
        }
    ...
    private void processKeyEvent(KeyEvent ev) {
        ...
        // While an OSR popup is open, route ALL keys to it (arrow/Enter/Esc/
        // type-ahead navigate the <select>/datalist list) — a native popup grabs
        // the keyboard, so the page (and the editing shortcuts below) don't see
        // keys until it closes. Mirrors the mouse/wheel popup routing.
        double[] popupRect = page.getPopupRect();
        if (popupRect != null) {
            String pkText = (ev.getEventType() == KeyEvent.KEY_TYPED)
                ? ev.getCharacter() : null;
            int pkWvk = (ev.getEventType() == KeyEvent.KEY_TYPED)
                ? 0 : KeyCodeMap.lookup(ev.getCode()).getWindowsVirtualKeyCode();
            WCKeyEvent pk = new WCKeyEvent(
                ID_MAP.get(ev.getEventType()), pkText,
                (ev.getEventType() == KeyEvent.KEY_TYPED)
                    ? null : KeyCodeMap.lookup(ev.getCode()).getKeyIdentifier(),
                pkWvk,
                ev.isShiftDown(), ev.isControlDown(),
                ev.isAltDown(), ev.isMetaDown(), System.currentTimeMillis());
            page.dispatchPopupKeyEvent(pk);
            ev.consume();
            return;
        }

        // Editing shortcuts → run the editor command in the engine (reliable in the
        // off-screen setup) and DON'T also forward the raw key (avoids e.g. double
        // paste). Ctrl on Win/Linux, Cmd (meta) on macOS; AltGr (ctrl+alt) char
        // combos are excluded. The control-character KEY_TYPED that trails an
        // accelerator is swallowed so it isn't injected as text.
        boolean accel = (ev.isControlDown() || ev.isMetaDown()) && !ev.isAltDown();
        if (accel) {
            if (ev.getEventType() == KeyEvent.KEY_PRESSED) {
                int cmd = editingCommandFor(ev.getCode(), ev.isShiftDown());
                if (cmd >= 0) {
                ...
                ev.consume();   // suppress the control char (e.g. Ctrl+C → '')
                return;
            }
        }
    ...
    /** Maps an accelerator key to a WebEngine editor-command code, or -1 if none. */
    private static int editingCommandFor(KeyCode code, boolean shift) {
        return switch (code) {
            case C -> WebEngine.EDIT_COPY;
            case X -> WebEngine.EDIT_CUT;
            case V -> WebEngine.EDIT_PASTE;
            case A -> WebEngine.EDIT_SELECT_ALL;
            case Z -> shift ? WebEngine.EDIT_REDO : WebEngine.EDIT_UNDO;
            case Y -> WebEngine.EDIT_REDO;
            default -> -1;
        };
    }
    ...
    private void registerEventHandlers() {
        ...
        // Consume JavaFX's own right-click ContextMenuEvent so no stray menu shows
        // here. The actual context menu is driven by the engine (Blink's
        // HandleContextMenu), which fires only when the page didn't preventDefault —
        // so pages with their own menu are respected. The right-click mouse events
        // are still forwarded to the engine by processMouseEvent.
        addEventFilter(ContextMenuEvent.CONTEXT_MENU_REQUESTED, ContextMenuEvent::consume);
