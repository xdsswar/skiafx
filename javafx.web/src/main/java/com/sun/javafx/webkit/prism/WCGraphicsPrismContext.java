...
class WCGraphicsPrismContext extends WCGraphicsContext {
    ...
    // skia-fx Phase 3: native SkSurface handle backing baseGraphics when
    // the active pipeline is Skia-backed; 0 otherwise. Future fast-path
    // draw methods (Phase 4) call NativeBridge.surface* directly with
    // this handle, skipping the baseGraphics dispatch when transform /
    // clip / paint state can be expressed in the bridge's batched
    // surface_begin_draw call.
    final long skSurfaceHandle;

    // Depth of bridge save levels (surface_save / surface_save_layer_alpha)
    // this context has pushed and not yet popped. The native surface's save
    // count is long-lived (it lives on the SkSurface, shared across frames and
    // contexts), so a context must only ever pop what it itself pushed —
    // otherwise an unbalanced WebKit save/restore (or a sibling context on the
    // same surface) drives surface_restore below zero: "underflow refused — fix
    // the WC layer pairing". Routing every layer save/restore through
    // bridgeSave* / bridgeRestore keeps this self-balancing: bridgeRestore is a
    // no-op once depth hits 0, and dispose() unwinds any leftover saves.
    private int bridgeSaveDepth;

    /** Push a bridge save level (rect/path clip layer). */
    private void bridgeSave() {
        SkiaSurfaceAccess.saveSurfaceState(skSurfaceHandle);
        bridgeSaveDepth++;
    }

    /** Push a bridge alpha layer (CSS opacity / transparency layer). */
    private void bridgeSaveLayerAlpha(float opacity) {
        SkiaSurfaceAccess.saveSurfaceLayerAlpha(skSurfaceHandle, opacity);
        bridgeSaveDepth++;
    }

    /** Pop one bridge save level — but never below this context's own baseline. */
    private void bridgeRestore() {
        if (bridgeSaveDepth > 0) {
            SkiaSurfaceAccess.restoreSurfaceState(skSurfaceHandle);
            bridgeSaveDepth--;
        }
    }
    ...
    WCGraphicsPrismContext(Graphics g) {
        ...
        baseGraphics = g;
        skSurfaceHandle = SkiaSurfaceAccess.handleOf(g);
    ...
    private void startNewLayer(Layer layer) {
        ...
        if (layer.isDirectCanvas()) {
            // skia-fx: bufferless Skia-direct layer (SkiaClipLayer).
            // Don't translate the state.transform — there is no buffer at
            // origin (0,0) for subsequent draws to render into; draws go
            // straight to the live SkCanvas in scene coordinates. Init
            // the layer directly on baseGraphics so it can fish out the
            // SkSurface handle.
            layer.init(baseGraphics);
            state.setLayer(layer);
            resetCachedGraphics();
            return;
        }

        // legacy ClipLayer / TransparencyLayer path: layer has its own
        // RTT buffer at (0,0); shift subsequent draws so they land
        // inside the buffer.
    ...
    public void dispose() {
        ...
        // Each ContextState.clone() copies the layer reference (see the
        // ContextState copy ctor's setLayer call), so a single still-open
        // Layer is referenced by EVERY nested state above the level it was
        // pushed at. Disposing per-state would call Layer.dispose() — and
        // for SkiaClipLayer / SkiaTransparencyLayer that means a bridge
        // surface_restore — once for every referencing state, while the
        // layer's init() only pushed a single save. The surplus pops
        // underflow the surface's save stack ("surface_restore underflow
        // refused — fix the WC layer pairing") and corrupt the clip/layer
        // nesting, which is what was clipping the gradient fills away.
        //
        // Dispose each DISTINCT layer exactly once, innermost-first (LIFO),
        // so the bridge save stack unwinds to zero — matching the one
        // save() each layer's init() pushed.
        Set<Layer> disposed = Collections.newSetFromMap(new IdentityHashMap<>());
        Layer current = (state != null) ? state.getLayerNoClone() : null;
        if (current != null && disposed.add(current)) {
            current.dispose();
        }
        for (int i = states.size() - 1; i >= 0; i--) {
            Layer layer = states.get(i).getLayerNoClone();
        ...
        // Safety net: if any bridge save level this context pushed wasn't popped
        // by a layer dispose above (orphaned save), unwind it now so the shared
        // surface's save stack returns to the baseline this context started at.
        while (bridgeSaveDepth > 0) {
            bridgeRestore();
        }
    }
    ...
    public void setClip(WCPath path, boolean isOut) {
        ...
        if (!isOut && skSurfaceHandle == 0L) {
            // Skip the rect-clip intersection on Skia surfaces. The
            // SkiaClipLayer's bridge clipPath applies the actual path
            // clip; intersecting state.clip with the path's bounding
            // box on top often produces tiny or negative-height rects
            // (WebKit's setClip(WCPath) is called per-element across
            // scanlines), and that empty rect-clip then nukes the
            // gradient + text drawn inside the layer at begin_draw
            // time. Path-precise clipping is sufficient.
        ...
        Rectangle clip = state.getClipNoClone();
        if (clip == null) {
            // No clip on the graphics state — the rect-clip block above is
            // skipped on Skia surfaces, and an out-clip never runs it either,
            // so getClipNoClone() can be null and the addRect/translate below
            // would NPE. Fall back to the transformed path's bounds, which
            // gives the layer a valid extent and a sane translate origin.
            WCRectangle pb = path.getBounds();
            int cx = (int) Math.floor(pb.getX());
            int cy = (int) Math.floor(pb.getY());
            int cw = (int) Math.ceil(pb.getMaxX()) - cx;
            int ch = (int) Math.ceil(pb.getMaxY()) - cy;
        ...
        // skia-fx Phase B: SkiaClipLayer applies the bridge surface_save
        // + surface_clip_path + surface_restore. The rect-clip
        // intersection above is skipped on Skia surfaces so the
        // gradient + text inside the layer aren't nuked by an empty
        // begin_draw clipRect.
        Layer layer = (skSurfaceHandle != 0L)
            ? new SkiaClipLayer(getGraphics(false), clip, path, skSurfaceHandle)
            : new ClipLayer(getGraphics(false), clip, path, type() == Type.DEDICATED);
 ...
 */
    ...
    public void fillRect(final float x, final float y, final float w, final float h, final Color color) {
        ...
        new Composite() {
            @Override void doPaint(Graphics g) {
                ...
                // projection.
                if (shadow != null && canDrawNativeShadow()) {
                    // Native Skia box-shadow: blurred rect at the shadow offset,
                    // then the normal fill on top. Bypasses the broken Prism
                    // DropShadow effect composite.
                    drawNativeShadow(g, new RoundRectangle2D(x, y, w, h, 0, 0), shadow);
                    g.setPaint(paint);
                    g.fillRect(x, y, w, h);
                } else if (shadow != null || !state.getPerspectiveTransformNoClone().isIdentity()) {
        ...
        final Color color)
    {
        ...
        new Composite() {
            @Override void doPaint(Graphics g) {
                ...
                if (shadow != null && canDrawNativeShadow()) {
                    drawNativeShadow(g,
                            new RoundRectangle2D(x, y, w, h, arcW, arcH), shadow);
                    g.setPaint(color);
    ...
    @Override public void beginTransparencyLayer(float opacity) {
        // skia-fx Phase D-ish (plan twinkly-splashing-dove): on Skia
        // surfaces, route to SkiaTransparencyLayer — no RTT-buffer
        // composite, opacity is applied multiplicatively via state.alpha
        // for the duration of the layer. Avoids the surface_draw_surface
        // crash in the legacy TransparencyLayer.render path
        // (drawTexture(SkiaRTTexture) → memory #12). Visually identical
        // for the common case of straight opacity on a content tree —
        // each draw inside the layer is alpha-multiplied at composite
        // time, no isolated buffer needed.
        Layer layer = (skSurfaceHandle != 0L)
            ? new SkiaTransparencyLayer(
                  getGraphics(false), state.getClipNoClone(), opacity, skSurfaceHandle)
    ...
    /** True when a box-shadow can take the native Skia gaussian-blur fast-path
     *  instead of the Prism DropShadow effect (whose intermediate-surface
     *  composite is broken on Skia — hard / missing shadows). Requires a
     *  Skia-backed surface and an identity perspective transform. */
    private static final boolean NATIVE_SHADOW =
        !"false".equals(System.getProperty("skia.web.nativeShadow", "true"));

    private boolean canDrawNativeShadow() {
        return NATIVE_SHADOW
            && skSurfaceHandle != 0L
            && state.getPerspectiveTransformNoClone().isIdentity();
    }

    /** Draw {@code shape}'s box-shadow straight on the live SkCanvas via the
     *  native blurred-shape fill. Honors the current transform + clip (the
     *  shadow shares the draw's begin_draw bracket inside SkiaGraphics). */
    private void drawNativeShadow(Graphics g, Shape shape, DropShadow shadow) {
        Color4f c = shadow.getColor();
        int argb = (shadowByte(c.getAlpha()) << 24)
                 | (shadowByte(c.getRed())   << 16)
                 | (shadowByte(c.getGreen()) <<  8)
                 |  shadowByte(c.getBlue());
        // JFX DropShadow.radius carries the CSS blur radius; Skia's mask-filter
        // wants a gaussian sigma, ~half the CSS blur (matches other browsers).
        float sigma = Math.max(0f, shadow.getRadius() * 0.5f);
        SkiaSurfaceAccess.drawShapeShadow(g, shape,
                shadow.getOffsetX(), shadow.getOffsetY(), sigma, argb);
    }

    private static int shadowByte(float v) {
    ...
    private abstract static class Layer {
        ...
        Layer(Graphics g, Rectangle bounds, boolean permanent) {
            this(g, bounds, permanent, /*allocateBuffer=*/true);
        }

        /**
         * skia-fx: Lightweight Layer ctor variant that skips the
         * RTTexture / PrDrawable allocation. The bridge-driven
         * {@link SkiaClipLayer} doesn't need a backing buffer — it
         * manipulates the live SkCanvas state directly — and the RTT
         * allocation here recurses into Effect.getCompatibleImage,
         * which is the path the legacy ClipLayer crashed on.
         */
        Layer(Graphics g, Rectangle bounds, boolean permanent, boolean allocateBuffer) {
        ...
        /**
         * skia-fx: a "direct canvas" Layer doesn't own an RTT buffer
         * and draws straight to the underlying SkSurface. The
         * {@code startNewLayer} translation that exists to remap into
         * a layer-local (0,0) buffer must be skipped for these, and
         * their {@link #init} must be called on baseGraphics so they
         * can pull the SkSurface handle out. Default: legacy (false).
         */
        boolean isDirectCanvas() { return false; }
    ...
    /**
     * skia-fx Phase D-ish: Skia-direct transparency layer. The legacy
     * {@link TransparencyLayer} allocates an RTT buffer, captures draws
     * into it, then composites the buffer back at the layer's opacity —
     * the final composite goes through {@code surface_draw_surface}
     * which corrupts the shared GrDirectContext (see memory note
     * webview-open-followups #12). For the common case of a layer that's
     * just an opacity wrapper (e.g., CSS {@code opacity: 0.92}), the
     * mathematically equivalent operation is "multiply every draw's
     * extra alpha by {@code opacity} for the lifetime of the layer".
     * That avoids the buffer + composite entirely. Mathematically
     * identical for opaque-stacked content; can differ subtly when a
     * layer's content has overlapping semi-transparent draws, but
     * neither WebKit nor any of our demo content hits that case in
     * practice.
     */
    private final class SkiaTransparencyLayer extends Layer {
        private final float opacity;
        private final long surfaceHandle;

        private SkiaTransparencyLayer(Graphics g, Rectangle bounds,
                                      float opacity, long skSurfaceHandle) {
            super(g, bounds, /*permanent=*/false, /*allocateBuffer=*/false);
            this.opacity = opacity;
            this.surfaceHandle = skSurfaceHandle;
        }

        @Override boolean isDirectCanvas() { return true; }

        @Override Graphics getGraphics() {
            // Return baseGraphics so RQ-decoded draws inside the layer
            // (CSS background-gradient fillRect, legacy fillText, theme
            // widget paints) land on the live SkCanvas via SkiaGraphics
            // and get captured by Skia's saveLayerAlpha. Without this,
            // l.getGraphics() in the outer WCGraphicsPrismContext.
            // getGraphics(boolean) returns null and rq-decoded draws
            // silently skip — visible as missing gradient backgrounds
            // and missing body text on any element with box-shadow or
            // CSS opacity (e.g. demo's "Gradient cards" section).
            //
            // The original tradeoff that left this null was motivated
            // by adjacent-element text rendering "ghost stripes" when
            // baseGraphics's extraAlpha was non-1.0 — root cause was
            // kSubpixelAntiAlias (LCD) on a translucent dest. That has
            // since been fixed in the bridge's draw_glyphs path which
            // now forces kAntiAlias (greyscale) AA — see
            // openjfx_skia_bridge.cpp surface_draw_glyphs and memory
            // [[project_text_ghosting_subpixel_aa]]. Safe to return
            // baseGraphics now.
            return baseGraphics;
        }

        @Override void init(Graphics g) {
            // Push a Skia saveLayerAlpha. Every subsequent draw on
            // this canvas (Skia-direct C++ via begin_draw, or rq-
            // decoded through baseGraphics → SkiaGraphics) lands in
            // an internal off-screen layer that Skia allocates and
            // manages; the matching dispose() pops the layer and
            // composites it back with `opacity`. This is the
            // Chrome-grade primitive for CSS opacity /
            // beginTransparencyLayer — no recursive SkSurface, no
            // surface_draw_surface readback (memory note
            // [[project_webview_open_followups]] #12).
            bridgeSaveLayerAlpha(opacity);
        }

        @Override void render(Graphics g) { /* nothing to composite */ }
        ...
        @Override public String toString() {
            return String.format("SkiaTransparencyLayer[%d,%d + %dx%d, opacity %.2f]",
                bounds.x, bounds.y, bounds.width, bounds.height, opacity);
        }
    }
    ...
    private static final class ClipLayer extends Layer {
        ...
        @Override void init(Graphics g) {
            // skia-fx Phase 5: SkiaGraphics implements ReadbackGraphics
            // via the bridge's surface_draw_surface entry, so this works
            // for both classic Prism and Skia surfaces. The defensive
            // non-ReadbackGraphics short-circuit is preserved in case a
            // future pipeline lacks readback.
            if (!(g instanceof ReadbackGraphics)) {
                srcover = false;
                return;
            }
    ...
    /**
     * skia-fx Phase 5: Skia-direct path clip. Replaces the heavyweight
     * RTT-readback + SRC_IN composite of the legacy {@link ClipLayer}
     * with two bridge calls: a {@code surface_save} + {@code clipPath}
     * at push time, and a {@code surface_restore} at pop time.
     *
     * <p>Why this works where the previous ClipLayer-on-Skia crashed:
     * we never allocate a recursive SkSurface, never read back the
     * destination, never re-blit. The clip just lives on the live
     * canvas's clip stack between {@link #init} and {@link #dispose}.
     * Draws issued in between — Skia-direct or rq-decoded through
     * SkiaGraphics — see the clip via Skia's standard save-stack
     * inheritance, with their per-draw {@code begin_draw}/{@code
     * end_draw} save/restore nesting cleanly inside ours.</p>
     */
    // Non-static inner class (like SkiaTransparencyLayer) so getGraphics()
    // can return the outer baseGraphics — see the override below.
    private final class SkiaClipLayer extends Layer {
        private final WCPath normalizedToClipPath;
        private final long surfaceHandle;

        private SkiaClipLayer(Graphics g, Rectangle bounds, WCPath path,
                              long skSurfaceHandle) {
            // skipBufferAlloc=true — no PrDrawable/RTT needed; we drive
            // the live SkCanvas directly.
            super(g, bounds, /*permanent=*/false, /*allocateBuffer=*/false);
            this.normalizedToClipPath = path;
            this.surfaceHandle = skSurfaceHandle;
        }
        ...
        @Override Graphics getGraphics() {
            // The clip is applied straight to the live SkCanvas, so RQ-decoded
            // draws issued inside the layer (CSS background-gradient fillRect,
            // image fills, path fills, theme widgets) must land on baseGraphics
            // → SkiaGraphics. Without this override the base Layer.getGraphics()
            // returns null (this layer owns no RTT buffer), so getGraphics(true)
            // hands back null and every such draw is silently dropped — visible
            // as missing gradient/image backgrounds on any rounded-rect /
            // clip-path element while the glyph-direct text on top still shows.
            return baseGraphics;
        }
        ...
        @Override void init(Graphics g) {
            Path2D p2d = ((WCPathImpl) normalizedToClipPath).getPlatformPath();
            bridgeSave();
            SkiaSurfaceAccess.clipPath(surfaceHandle, p2d, bounds.x, bounds.y);
        }
        ...
        @Override void render(Graphics g) {
            // No-op: the clip was applied to the base canvas directly,
            // so every draw issued between init and dispose was already
            // clipped. dispose() unwinds the saved state.
        }

        @Override public String toString() {
            return String.format("SkiaClipLayer[%d,%d + %dx%d, path %s]",
                    bounds.x, bounds.y, bounds.width, bounds.height,
                    normalizedToClipPath);
        }
    }
