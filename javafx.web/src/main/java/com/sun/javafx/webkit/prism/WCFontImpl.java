...
import com.sun.prism.skia.SkiaTypeface;
...
final class WCFontImpl extends WCFont {
    ...
    // skia-fx Phase 4 (batch 5): lazy SkTypeface lookup. The actual
    // typeface lives in SkiaTypeface's process-wide cache (keyed by font
    // file path), so this field is just a one-time-per-WCFont fetch
    // that caches the long handle for native callers.
    private SkiaTypeface skTypeface;
    private boolean skTypefaceResolved;
    ...
    @Override
    public synchronized long getOrCreateSkTypefaceHandle() {
        if (!skTypefaceResolved) {
            skTypeface = SkiaTypeface.forFontResource(font.getFontResource());
            skTypefaceResolved = true;
        }
        return skTypeface == null ? 0L : skTypeface.getNativeHandle();
    }
