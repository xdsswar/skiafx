...
final class WCImageImpl extends PrismImage {
    ...
    // skia-fx Phase 4: lazy SkImage handle for the native bridge's
    // surface_draw_image_rect fast path. 0 = not yet uploaded /
    // pipeline not Skia. Released in dispose().
    private long skImageHandle;
    ...
    void dispose() {
        ...
        // Synchronize on the same monitor as getOrCreateSkImageHandle(): without
        // it, a concurrent create could publish a fresh handle that this dispose
        // then leaks, or both could race to destroy the same handle.
        synchronized (this) {
            if (skImageHandle != 0L) {
                SkiaImageAccess.destroyImage(skImageHandle);
                skImageHandle = 0L;
            }
        }
    }
    ...
    @Override
    public synchronized long getOrCreateSkImageHandle() {
        if (skImageHandle != 0L) {
            return skImageHandle;
        }
        long h = SkiaImageAccess.createRasterImage(
            img.getWidth(), img.getHeight(), img.getScanlineStride(),
            img.getPixelFormat(), img.getPixelBuffer());
        skImageHandle = h;
        return h;
    }
