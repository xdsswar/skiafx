...
class WCWidget {
    ...
    // skia-fx Blink: initIDs() cached jfieldIDs for the retired jfxwebkit
    // native library, which is no longer loaded — calling it on class load
    // would throw UnsatisfiedLinkError. The declaration is kept (unused) so
    // the rest of the class is untouched; the static initializer is removed.
