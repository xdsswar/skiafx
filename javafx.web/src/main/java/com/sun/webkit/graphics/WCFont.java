...
public abstract class WCFont extends Ref {
    ...
    /**
     * Returns the native {@code SkTypeface} handle for this font, lazily
     * created on first call, or {@code 0L} if the underlying font has no
     * usable file (system synthetic fonts, decorated fonts without a
     * source TTF/OTF/TTC). Callers must NOT release the handle — it is
     * owned by the JVM-wide {@link com.sun.prism.skia.SkiaTypeface}
     * cache and lives for the process lifetime.
     */
    public long getOrCreateSkTypefaceHandle() { return 0L; }

    /**
     * Returns this font's size in points (CSS px). Needed by the native
     * skia-direct text path to call {@code SkFont(typeface, size)}.
     */
