...
public abstract class CursorManager<T> {
    ...
    // Public so the Blink off-screen path (com.sun.webkit.WebPage) can resolve a
    // cursor type forwarded by the engine to an id, then hand it to the page
    // client. Native WebKit reached this via JNI; the Blink path is in-VM. Still
    // invoked by native WebKit too — keep the signature.
    public long getPredefinedCursorID(int type) {
