...
public final class MediaPlayer {
    ...
    /**
     * Skia-fx: describes whether this player operates on a single
     * multiplexed source or a pair of separate audio + video sources.
     * Determined at construction time from the {@link Media} the
     * player was built with — see {@link #playerModeProperty()}.
     */
    public enum PlayerMode {
        /** Single-source playback — audio (if any) is in the same
         *  container as the video. The classic JavaFX media model.
         *  Set when {@link Media#getAudioSource()} returns {@code null}. */
        SINGLE,
        /** Dual-source playback — audio and video come from separate
         *  URLs and are played in parallel by two internal pipelines.
         *  Set when {@link Media} was built with the dual-source
         *  constructors {@code Media(audio, video)} or
         *  {@code Media(audio, video, headers)}. */
        DUAL
    }

    /** Read-only property exposing the player mode. The value is
     *  set once at construction and never changes for the player's
     *  lifetime. */
    private final ReadOnlyObjectWrapper<PlayerMode> playerMode;

    /** @return the current player mode. Never {@code null}. */
    public final PlayerMode getPlayerMode() {
        return playerMode.get();
    }

    /** @return the read-only property holding the player mode. Bind to
     *  this to react to whether the player is in single or dual mode
     *  (e.g. for UI elements that should only show in dual-stream
     *  configurations). */
    public final ReadOnlyObjectProperty<PlayerMode> playerModeProperty() {
        return playerMode.getReadOnlyProperty();
    }

    // The underlying player
    private com.sun.media.jfxmedia.MediaPlayer jfxPlayer;

    // Skia-fx dual-source NOTE:
    //
    // Dual-source playback (Media built with a separate audio source
    // via new Media(audioUrl, videoUrl[, headers])) is handled
    // ENTIRELY in the native GStreamer pipeline — there is no second
    // Java MediaPlayer. The native pipeline accepts the companion
    // audio URL from the Locator, builds a second source-bin inside
    // the same GstPipeline, and lets GStreamer's shared GstClock
    // sync the two sources at the sample level. That gives true
    // sub-frame lip-sync without any Java-side seek-based correction.
    //
    // Earlier iterations of this class kept a parallel jfxAudioPlayer
    // and mirrored every transport command + ran a periodic AV-sync
    // watcher. That approach hit fundamental limits — two independent
    // pipelines have independent clocks and drift by tens of ms within
    // seconds — and was removed when the native dual-source path
    // landed. The Java side now does exactly what it did before for
    // single-source: holds ONE native player. The DUAL flag exists
    // (PlayerMode property below) so apps can introspect the mode,
    // but functionally play/pause/stop/seek/rate all dispatch to the
    // single native player which is responsible for both sources.
    ...
    public MediaPlayer(@NamedArg("media") Media media) {
        ...
        // Skia-fx: lock the player mode at construction time. A Media
        // built with a separate audio companion source puts this
        // player into DUAL mode; otherwise SINGLE. The mode is fixed
        // for the lifetime of this MediaPlayer — read via
        // {@link #playerModeProperty()} or {@link #getPlayerMode()}.
        PlayerMode initialMode = (media.getAudioSource() != null
                                  && !media.getAudioSource().isEmpty())
                ? PlayerMode.DUAL
                : PlayerMode.SINGLE;
        playerMode = new ReadOnlyObjectWrapper<>(this, "playerMode", initialMode);
