...
public class PrismMediaFrameHandler implements ResourceFactoryListener {
    ...
    private class PrismFrameBuffer implements MediaFrame {
        ...
        // ----- Platform-GPU-texture passthrough --------------------------
        // The wrapped VideoDataBuffer is the authoritative source: a
        // hardware-decoded frame returns a non-zero handle here; software
        // frames return KIND_NONE / 0L and the existing plane-buffer path
        // wins. VideoDataBuffer.getPlatformTextureKind() defaults to
        // KIND_NONE so frames produced by upstream code that doesn't
        // know about the capability stay safe.

        @Override
        public int getPlatformTextureKind() {
            return primary.getPlatformTextureKind();
        }

        @Override
        public long getPlatformTextureHandle() {
            return primary.getPlatformTextureHandle();
        }

        @Override
        public int getPlatformTextureSubresource() {
            return primary.getPlatformTextureSubresource();
        }

        @Override
        public int getPlatformTextureWidth() {
            return primary.getPlatformTextureWidth();
        }

        @Override
        public int getPlatformTextureHeight() {
            return primary.getPlatformTextureHeight();
        }

        @Override
        public int getYuvColorSpace() {
            return primary.getYuvColorSpace();
        }

        @Override
        public int getColorTransfer() {
            return primary.getColorTransfer();
        }

        @Override
        public int getColorPrimaries() {
            return primary.getColorPrimaries();
        }

        @Override
        public int getColorRange() {
            return primary.getColorRange();
        }

        @Override
        public float getMasteringPeakNits() {
            return primary.getMasteringPeakNits();
        }
    }
