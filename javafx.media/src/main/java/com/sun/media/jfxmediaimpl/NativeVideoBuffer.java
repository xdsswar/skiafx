...
final class NativeVideoBuffer implements VideoDataBuffer {
    ...
    // M3-B: optional GPU-resident texture (HW-decoded frames). Returns
    // KIND_NONE / 0 for software frames so the existing plane-buffer
    // path stays the source of truth.
    private native int  nativeGetPlatformTextureKind(long handle);
    private native long nativeGetPlatformTextureHandle(long handle);
    private native int  nativeGetPlatformTextureSubresource(long handle);
    private native int  nativeGetPlatformTextureWidth(long handle);
    private native int  nativeGetPlatformTextureHeight(long handle);

    // skia-fx: YUV colour-space hint lifted from the demuxer's caps
    // (GstCaps "colorimetry" field). Returns -1 (AUTO) when no caps
    // hint is available — the consumer side then uses its own
    // resolution-based heuristic.
    private native int  nativeGetYuvColorSpace(long handle);

    // skia-fx HDR: full source colour descriptor lifted from caps.
    // All four return the SENTINEL_AUTO equivalent (-1 / 0) when the
    // demuxer didn't carry the field. The consumer combines these
    // with its resolution heuristic to decide whether to take the
    // HDR tone-mapping path.
    private native int   nativeGetColorTransfer(long handle);    // VideoDataBuffer.TRANSFER_*
    private native int   nativeGetColorPrimaries(long handle);   // VideoDataBuffer.PRIMARIES_*
    private native int   nativeGetColorRange(long handle);       // VideoDataBuffer.RANGE_*
    private native float nativeGetMasteringPeakNits(long handle);// 0.0 = unknown
    ...
    @Override
    public int getPlatformTextureKind() {
        if (0 != nativePeer) {
            return nativeGetPlatformTextureKind(nativePeer);
        }
        return PLATFORM_TEXTURE_KIND_NONE;
    }

    @Override
    public int getYuvColorSpace() {
        if (0 != nativePeer) {
            return nativeGetYuvColorSpace(nativePeer);
        }
    ...
    public int getColorTransfer() {
        if (0 != nativePeer) {
            return nativeGetColorTransfer(nativePeer);
        }
    ...
    public int getColorPrimaries() {
        if (0 != nativePeer) {
            return nativeGetColorPrimaries(nativePeer);
        }
    ...
    public int getColorRange() {
        if (0 != nativePeer) {
            return nativeGetColorRange(nativePeer);
        }
        return RANGE_AUTO;
    }

    @Override
    public float getMasteringPeakNits() {
        if (0 != nativePeer) {
            return nativeGetMasteringPeakNits(nativePeer);
        }
        return 0.f;
    }

    @Override
    public long getPlatformTextureHandle() {
        if (0 != nativePeer) {
            return nativeGetPlatformTextureHandle(nativePeer);
        }
        return 0L;
    }

    @Override
    public int getPlatformTextureSubresource() {
        if (0 != nativePeer) {
            return nativeGetPlatformTextureSubresource(nativePeer);
        }
        return 0;
    }
    ...
    public int getPlatformTextureWidth() {
        if (0 != nativePeer) {
            return nativeGetPlatformTextureWidth(nativePeer);
        }
        return 0;
    }
    ...
    public int getPlatformTextureHeight() {
        if (0 != nativePeer) {
            return nativeGetPlatformTextureHeight(nativePeer);
        }
        return 0;
    }
