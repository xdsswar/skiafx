    ...
    protected NativeMediaManager() {
        ...
        try {
            ...
            if (PlatformUtil.isWindows()) {
                // Order doesn't matter for correctness — NativeLibLoader
                // extracts EVERY dep to the same cache dir before
                // System.load fires on jfxmedia.dll, and Windows DLL
                // loader resolves jfxmedia's imports from that dir.
                // What DOES matter is that every transitive native
                // dependency is listed: missing one means it won't be
                // extracted from the in-jar resources and the load
                // chain breaks at runtime for downstream consumers
                // that ship our jars without -Djava.library.path.
                //
                // jfxmedia.dll → fxplugins.dll + glib-lite.dll
                // fxplugins.dll → gstreamer-lite.dll + glib-lite.dll
                // gstreamer-lite.dll → glib-lite.dll
                dependencies.add("fxplugins");
                dependencies.add("gstreamer-lite");
                dependencies.add("glib-lite");
            }
        ...
        // Auto-initialize the runtime-dynamic ffmpeg loader from the
        // standard JVM system property (`-Dopenjfx.media.ffmpeg.dir=…`)
        // or env var (`OPENJFX_MEDIA_FFMPEG_DIR`). End-user applications
        // configure ffmpeg-backed decode by setting the system property
        // — they should NOT need to call any com.sun.* API explicitly.
        // Both lookups are internal to MediaFfmpegConfig.initialize.
        // Failure is silent and harmless: ffmpegwrapper's is-supported
        // probe returns FALSE for every codec and routing falls through
        // to mfwrapper / dshowwrapper.
        try {
            MediaFfmpegConfig.initialize(null);
