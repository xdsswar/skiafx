...
public final class GSTPlatform extends Platform {
    ...
    private static final String[] CONTENT_TYPES = {
        ...
        "audio/mpegurl",
        // skia-fx: matroska + webm. Both go through matroskademux which
    ...
    public MediaPlayer createMediaPlayer(Locator source) {
        ...
        try {
            ...
            // Catch Throwable (not just Exception) — Errors like
            // UnsatisfiedLinkError from missing native libs or
            // OutOfMemoryError from a botched pipeline build need to
            // be surfaced too, otherwise the top-level "Could not
            // create player!" gives no hint at the real cause.
            if (Logger.canLog(Logger.DEBUG)) {
                Logger.logMsg(Logger.DEBUG,
                    "GSTPlatform caught exception while creating media player: " + e);
            }
            return null;
        }
