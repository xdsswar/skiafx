...
public class Locator {
    ...
    /**
     * Skia-fx: HTTP request headers carried from {@code javafx.scene
     * .media.Media} into the native pipeline. The native http source
     * (souphttpsrc on Linux/macOS, javahttpsrc on Windows) reads
     * these from the Locator via {@link #getHttpHeaders()} when it's
     * constructed. Always non-null; empty when nothing was set.
     */
    private final LinkedHashMap<String, String> httpHeaders =
        new LinkedHashMap<>();

    /** Skia-fx: per-Locator User-Agent. {@code null} = engine default. */
    private volatile String userAgent;

    /**
     * Skia-fx: companion audio source URL. When non-null, the native
     * pipeline builds a second source bin (audio-only) inside the
     * same GstPipeline, with the primary Locator's URL handling
     * video. The two source bins share the pipeline's GstClock so
     * GStreamer's native sync infrastructure delivers sub-frame
     * lip-sync without any Java-side correction. Read by native via
     * the {@link #getCompanionAudioUrl()} JNI call.
     */
    private volatile String companionAudioUrl;

    /** Skia-fx: set the companion audio URL. {@code null} or empty
     *  reverts this Locator to single-source mode. */
    public void setCompanionAudioUrl(String url) {
        this.companionAudioUrl = (url != null && !url.isEmpty()) ? url : null;
    }

    /** Skia-fx: get the companion audio URL, or {@code null} for
     *  single-source. Called from native ({@code CLocator}) via JNI
     *  during pipeline construction. */
    public String getCompanionAudioUrl() {
        return companionAudioUrl;
    }

    /**
     * Skia-fx: lazily-resolved companion container content type, one of
     * the {@code MediaUtils.CONTENT_TYPE_*} constants. {@code null} until
     * first probed.
     */
     ...
     *
     * <p>Called from native ({@code CLocator}) during pipeline
     * construction, only for {@code http(s)://} companions — {@code
     * file://} companions stay on the native {@code filesrc} path. Returns
     * {@code null} when there is no companion. Inherits the primary
     * Locator's connection properties (headers / User-Agent) so an
     * authenticated companion stream works the same way the primary
     * does.</p>
     */
    public ConnectionHolder createCompanionAudioConnectionHolder()
            throws IOException, URISyntaxException {
        String url = companionAudioUrl;
    ...
    /**
     * Skia-fx: detect the companion audio container content type (one of
     * the {@code MediaUtils.CONTENT_TYPE_*} constants) so native can pick
     * the matching demuxer + decoder.
     *
     * <p>Uses the file signature (EBML / ISO-BMFF magic), which is
     * prefix-agnostic: a server {@code Content-Type: audio/webm} still
     * resolves to the {@code video/webm} container constant the native
     * dispatch understands. Result is cached. Returns {@code null} for
     * single-source, or {@link #DEFAULT_CONTENT_TYPE} when detection
     * fails (native then leaves the audio bin to default negotiation).</p>
     */
    public String getCompanionAudioContentType() {
        String url = companionAudioUrl;
        if (url == null || url.isEmpty()) {
            return null;
        }
    ...
    /** Skia-fx: replace this Locator's HTTP headers. */
    ...
    /** Skia-fx: read the current HTTP headers (snapshot copy). */
    ...
    /** Skia-fx: set the User-Agent for HTTP requests. */
    public void setUserAgent(String ua) {
        this.userAgent = (ua != null && !ua.isEmpty()) ? ua : null;
    }

    /** Skia-fx: current User-Agent, or {@code null} for engine default. */
