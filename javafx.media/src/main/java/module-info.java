...
module javafx.media {
    ...
    // Phase-4: ffmpeg runtime-dynamic loader config. Apps that want
    // to enable the ffmpeg decoder set the library directory via
    // com.sun.media.jfxmediaimpl.MediaFfmpegConfig.initialize(...).
    // Export only to consumers we know about for now (the demo). Any
    // real third-party app can use --add-exports / -opens to access
    // it; we'll promote the API to a public package once the surface
    // stabilises.
    exports com.sun.media.jfxmediaimpl to
        openjfx.samples.ensemble;
}
