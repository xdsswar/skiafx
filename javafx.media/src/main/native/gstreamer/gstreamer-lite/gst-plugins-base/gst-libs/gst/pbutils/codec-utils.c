...
/* skia-fx: opus + AV1 + MIME-codec helpers were stripped from the
 * upstream lite build. Re-enabled to support the matroska demuxer
 * (which builds opus caps from in-band codec data). Bodies are pure
 * C with no extra link deps. */
/**
    ...
    GstBuffer * comments)
{
  ...
  /*
   * skia-fx: avoid the "streamheader" GValue-array path here.
   *
   * This gstreamer-lite core is v1.14 while the matroska demuxer that
   * drives this call is v1.26.5. Building the streamheader array via
   * _gst_caps_set_buffer_array() does g_value_init(&array,
   * GST_TYPE_ARRAY), and GST_TYPE_ARRAY resolves to 0 in this build
   * context ("type id '0' is invalid" GObject CRITICAL). The resulting
   * GValue is left uninitialised, and the buffer pointer read out of it
   * is a 32-bit-truncated/sign-extended garbage pointer that crashes
   * gst_mini_object_is_writable with an access violation. This only
   * fires for Opus-in-WebM (companion audio).
   *
   * The OpusTags half of the streamheader is never consumed downstream,
   * and ffmpegwrapper (our only Opus consumer) reads the OpusHead from
   * "codec_data" first, falling back to "streamheader". So attach the
   * OpusHead as a single "codec_data" buffer via gst_caps_set_simple()
   * (a plain GST_TYPE_BUFFER field — the proven, ubiquitous path that
   * doesn't touch the broken GST_TYPE_ARRAY codepath). Opus decodes
   * identically, without the crash.
   */
  (void) dummy_comments;
  (void) comments;
  caps = gst_caps_make_writable (caps);
  gst_caps_set_simple (caps, "codec_data", GST_TYPE_BUFFER, header, NULL);
