...
/* skia-fx: copied verbatim from upstream gstreamer mainline
 * (https://github.com/GStreamer/gstreamer/blob/main/subprojects/gstreamer/plugins/elements/gstfilesrc.c).
 *
 * Reinstated here because the gstreamer-lite source drop strips the
 * filesrc implementation while keeping the registration line in
 * gstcoreelementsplugin.c — so `gst_element_factory_make("filesrc",
 * ...)` returns NULL at runtime. The Media(video, audio) dual-source
 * path needs filesrc for the companion file:// URL.
 *
 * Modifications: none. The mainline file uses <glib/gi18n-lib.h>
 * (glib's i18n, same as gstqueue.c in this directory) and includes
 * its own GST_ELEMENT_REGISTER_DEFINE — so it drops in cleanly.
 *
 * The matching change in gstcoreelementsplugin.c moves
 * GST_ELEMENT_REGISTER(filesrc, plugin) out of the
 * #ifndef GSTREAMER_LITE block so it's also called in the lite
 * build. */
