...
#else // GSTREAMER_LITE
  // skia-fx: filesrc is needed for the companion-audio source path
  // (Media(video, audio) dual-source). The element implementation is
  // already compiled in via gstfilesrc.c; only the registration line
  // was excluded from gstreamer-lite. Register it so factory-make
  // succeeds without dragging in the rest of the upstream coreelements.
  ret |= GST_ELEMENT_REGISTER (filesrc, plugin);
#endif // GSTREAMER_LITE
