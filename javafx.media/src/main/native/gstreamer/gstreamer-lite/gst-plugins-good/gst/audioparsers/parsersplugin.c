...
#endif // GSTREAMER_LITE
/* skia-fx: aacparse is needed cross-platform for the dual-source
 * media path. qtdemux on .m4a/.mp4 emits raw AAC frames; without
 * aacparse the downstream decoder (dshowwrapper / mfwrapper /
 * ffmpegwrapper) mis-frames the bitstream and the resulting PCM
 * plays at the wrong sample rate. */
#include "gstaacparse.h"
...
#endif // GSTREAMER_LITE
{
  ...
  return ret;
#else // GSTREAMER_LITE
  /* skia-fx: aacparse registered on all platforms in lite. Needed
   * for the dual-source path (Media(video, audio)) where qtdemux of
   * an .m4a/.mp4 companion emits raw AAC that must be re-framed
