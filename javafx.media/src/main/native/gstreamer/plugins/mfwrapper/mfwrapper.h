...
struct _GstMFWrapper
{
    ...
    // Colorimetry captured from input caps. mfwrapper itself never
    // changes the colour matrix / transfer / primaries on its samples,
    // so we forward whatever the upstream demuxer carried (qtdemux
    // reads MP4's colr atom; matroskademux reads MKV Colour element).
    // See dshowwrapper.h for the same field with a longer rationale.
