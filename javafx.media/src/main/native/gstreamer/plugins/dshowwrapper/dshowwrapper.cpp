...
int dshowwrapper_sink_event(int sinkEvent, void *pData, int size, sUserData *pUserData)
{
    ...
    case SINK_VIDEO_RESOLUTION:
        if (pData != NULL && size == sizeof(sVideoResolutionEvent))
        {
            ...
            {
                // Same "format = I420" fix as the initial caps push —
                // dynamic dimension updates must keep the correct
                // format tag, otherwise GstVideoFrame falls back to
                // YV12 plane ordering and the Skia YUV upload swaps
                // U/V (the amber-backlight bug, see the long comment
                // at the initial caps_event site).
            ...
            // Carry the upstream colorimetry through to the
            // re-negotiated caps. Same justification as the initial
            // push site.
            if (decoder->input_colorimetry != NULL)
            {
                gst_caps_set_simple(caps,
...
static gboolean dshowwrapper_load_decoder_h264(GstStructure *s, GstDShowWrapper *decoder)
{
    ...
    if (decoder->eOutputFormat[DEFAULT_OUTPUT_DS_STREAM_INDEX] == MEDIA_FORMAT_VIDEO_I420)
    {
        // CRITICAL: tag caps as "I420", not "YV12".
        //
        // The DirectShow decoder outputs I420 layout (Y, Cb (U), Cr (V) —
        // see MEDIASUBTYPE_I420 and `biCompression = '024I'` below) and
        // the offsets we set match that layout (offset-u before offset-v).
        // But the caps format string used to be "YV12", which lied to
        // the downstream consumer: GstVideoFrame.cpp:184 only sets
        // m_bIsI420 = true when format == "I420", and it's that flag
        // that triggers SwapPlanes(1, 2) to convert from the YV12-
        // style (Y, V, U) plane order it reads-by-default into the
        // I420-native (Y, U, V) order the rest of the pipeline (and
        // the Skia YUV upload) expects.
        //
        // With the old "YV12" tag, the Skia consumer's YUV upload
        // reads plane[1] as U/Cb when it actually held V/Cr — the
        // classic R↔B-shifted output that shows up as "amber back-
        // light" / blue skin / cyan foliage. mfwrapper.cpp:1229
        // already fixed this same bug; dshowwrapper had the same
        // miswiring sitting in the H.264 path until today.
        GstCaps *caps = NULL;
        ...
        // Forward the input colorimetry untouched. The DirectShow
        // decoder doesn't change colour metadata — it just produces
        // YUV samples in the same matrix/transfer/primaries the
        // container declared — so passing the caps field through
    ...
    {
        // DirectShow's YV12 layout is [Y][V][U] (V plane immediately
        // after Y, U plane last). The downstream YUV upload assumes
        // I420 plane order — so we still tag caps as "I420" (which
        // triggers GstVideoFrame's SwapPlanes(1, 2)) but point the
        // explicit "offset-v"/"offset-u" fields at the actual data
        // positions inside the YV12 buffer. After the swap, plane 1
        // ends up holding U/Cb and plane 2 V/Cr — what Skia expects.
        // Without this fix we'd ship YV12-ordered planes into a
        // pipeline that interprets them as I420 → U/V swap → the
        // amber-backlight / blue-skin colour bug.
        GstCaps *caps = NULL;
        caps = gst_caps_new_simple ("video/x-raw-yuv",
            "format", G_TYPE_STRING, "I420",
            "framerate", GST_TYPE_FRACTION, framerate_num, framerate_den,
            ...
            "height", G_TYPE_INT, height,
            "offset-y", G_TYPE_INT, 0,
            // YV12 layout: V plane comes first, then U.
            "offset-v", G_TYPE_INT, width*height,
            "offset-u", G_TYPE_INT, (width*height+((width*height)/4)),
            "stride-y", G_TYPE_INT, width,
            "stride-v", G_TYPE_INT, width/2,
            "stride-u", G_TYPE_INT, width/2,
            NULL);

        // Same forwarding as the I420 branch above — DirectShow's
        // YV12 vs I420 choice is a plane-order detail, not a colour-
        // space change. Forward the producer's colorimetry as-is.
        if (decoder->input_colorimetry != NULL)
        {
            gst_caps_set_simple(caps,
                "colorimetry", G_TYPE_STRING, decoder->input_colorimetry,
...
static gboolean dshowwrapper_sink_set_caps(GstPad * pad, GstObject *parent, GstCaps * caps)
{
    ...
    {
        // Lift the canonical "colorimetry" field from the input caps
        // BEFORE we configure DirectShow. qtdemux fills this in from
        // MP4's colr atom; matroskademux from MKV's Colour element;
        // the H.264 parser from VUI parameters. DirectShow itself
        // strips the metadata at decode time so we have to ferry it
        // around the decoder ourselves.
