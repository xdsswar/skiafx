...
struct _GstDShowWrapper
{
    ...
    // Colorimetry string captured from the input caps (qtdemux,
    // matroskademux etc. produce the canonical "R:M:T:P" form when
    // the container carries a colr atom / Colour element). We hold
    // the producer's value here so the src caps we push downstream
    // can mirror it, instead of dropping the colour metadata on the
    // floor (which forces the Skia consumer onto a resolution
    // heuristic — see com.sun.prism.skia.SkiaMediaTexture for the
    // fallback rules).  NULL = upstream didn't send one.
    gchar *input_colorimetry;
};
