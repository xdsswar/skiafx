...
void CVideoFrame::Reset()
{
    ...
    m_iPlatformTextureKind         = PLATFORM_TEXTURE_KIND_NONE;
    m_pvPlatformTextureHandle      = nullptr;
    m_uiPlatformTextureSubresource = 0;
    m_uiPlatformTextureWidth       = 0;
    m_uiPlatformTextureHeight      = 0;
    // Note: m_iYuvColorSpace is deliberately NOT reset here — it's
    // populated once per CAPS change in CGstVideoFrame::SetFrameCaps
    // and applies for the lifetime of that caps negotiation. Reset()
    // runs per-frame to clear plane-data state and doesn't need to
    // re-clear caps-level metadata.
}
