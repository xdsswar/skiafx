...
jstring CLocator::GetCompanionAudioUrl(JNIEnv *env, jobject locator)
{
    if (env == NULL || locator == NULL)
        return NULL;
    ...
    // Cache the methodID across calls — same pattern as the other
    // JNI accessors in this file. Looked up lazily on first use so
    // we don't pay the cost when single-source playback is the
    // common case.
    static jmethodID mid_GetCompanionAudioUrl = NULL;
    if (mid_GetCompanionAudioUrl == NULL)
    {
        jclass klass = env->GetObjectClass(locator);
        mid_GetCompanionAudioUrl = env->GetMethodID(klass,
            "getCompanionAudioUrl", "()Ljava/lang/String;");
        env->DeleteLocalRef(klass);
        if (javaEnv.clearException() || mid_GetCompanionAudioUrl == NULL)
        {
            // Older Java side without dual-source support — fall
            // through silently. Callers treat NULL as "no companion".
            return NULL;
        }
    }

    jstring result = (jstring)env->CallObjectMethod(locator, mid_GetCompanionAudioUrl);
    if (javaEnv.clearException())
        return NULL;
    return result;
}

jobject CLocator::GetCompanionAudioConnectionHolder(JNIEnv *env, jobject locator)
{
    if (env == NULL || locator == NULL)
        return NULL;
    ...
    static jmethodID mid_GetCompanionAudioConnectionHolder = NULL;
    if (mid_GetCompanionAudioConnectionHolder == NULL)
    {
        jclass klass = env->GetObjectClass(locator);
        mid_GetCompanionAudioConnectionHolder = env->GetMethodID(klass,
            "createCompanionAudioConnectionHolder",
            "()Lcom/sun/media/jfxmedia/locator/ConnectionHolder;");
        env->DeleteLocalRef(klass);
        if (javaEnv.reportException() || mid_GetCompanionAudioConnectionHolder == NULL)
            return NULL;
    }

    jobject holder = env->CallObjectMethod(locator, mid_GetCompanionAudioConnectionHolder);
    if (javaEnv.reportException())
        return NULL;
    return holder;
}

jstring CLocator::GetCompanionAudioContentType(JNIEnv *env, jobject locator)
{
    if (env == NULL || locator == NULL)
        return NULL;
    ...
    static jmethodID mid_GetCompanionAudioContentType = NULL;
    if (mid_GetCompanionAudioContentType == NULL)
    {
        jclass klass = env->GetObjectClass(locator);
        mid_GetCompanionAudioContentType = env->GetMethodID(klass,
            "getCompanionAudioContentType", "()Ljava/lang/String;");
        env->DeleteLocalRef(klass);
        if (javaEnv.clearException() || mid_GetCompanionAudioContentType == NULL)
            return NULL;
    }

    jstring result = (jstring)env->CallObjectMethod(locator, mid_GetCompanionAudioContentType);
    if (javaEnv.clearException())
        return NULL;
    return result;
}
