...
private:
    ...
    // Skia-fx: native source element for a companion audio URL
    // (Media's audioSource). Returns filesrc for file:// URLs,
    // souphttpsrc for http(s):// — no Java I/O bridge involved.
    uint32_t    CreateCompanionAudioSource(const std::string& url, GstElement** ppElement);
