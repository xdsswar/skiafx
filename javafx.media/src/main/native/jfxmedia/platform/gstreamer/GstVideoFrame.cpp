...
// M3-B media zero-copy meta — header-only, registers idempotently with
// the GStreamer meta registry so both mfwrapper.dll (producer) and
// jfxmedia.dll (consumer) end up with the same GstMetaInfo at runtime.
...
bool CGstVideoFrame::Init(GstSample* sample)
{
    ...
    // M3-B: surface the optional platform-GPU-texture meta if present.
    // No-op when no meta is attached (mfwrapper only attaches when
    // OPENJFX_MEDIA_D3D11_META=1 is set). Implementation lives in the
    // non-inline openjfx_media_d3d11_meta.cpp linked into jfxmedia.
    OpenJfxMediaD3d11Meta* d3dMeta =
        openjfx_media_d3d11_meta_get(m_pBuffer);

    if (d3dMeta != NULL && d3dMeta->d3d11Texture != NULL) {
        SetPlatformTexture(
            d3dMeta->kind,
            d3dMeta->d3d11Texture,
            d3dMeta->subresource,
            d3dMeta->width,
            d3dMeta->height);
    }
...
void CGstVideoFrame::SetFrameCaps(GstCaps *newCaps)
{
    ...
    // skia-fx: lift the YUV colour-space hint from the demuxer's caps
    // when present, so the Skia consumer-side upload can pick the
    // right matrix instead of guessing from resolution. GStreamer's
    // canonical encoding is the "colorimetry" string field — set by
    // qtdemux from MP4's colr atom, matroskademux from MKV's colour
    // element, etc. Accepts both the short tokens ("bt709", "bt601",
    // "bt2020", "smpte240m", "sRGB") and the long "R:M:T:P" form
    // (range:matrix:transfer:primaries). The substring match below
    // handles both cleanly.
    const gchar* colorimetry = gst_structure_get_string(str, "colorimetry");
    if (colorimetry != nullptr) {
        if (g_strrstr(colorimetry, "bt709") != nullptr) {
            m_iYuvColorSpace = YUV_COLORSPACE_BT709;
        } else if (g_strrstr(colorimetry, "bt2020") != nullptr) {
            m_iYuvColorSpace = YUV_COLORSPACE_BT2020;
        } else if (g_strrstr(colorimetry, "bt601")    != nullptr ||
                   g_strrstr(colorimetry, "smpte170") != nullptr ||
                   g_strrstr(colorimetry, "smpte240") != nullptr ||
                   g_strrstr(colorimetry, "bt470")    != nullptr) {
            m_iYuvColorSpace = YUV_COLORSPACE_BT601;
        } else if (g_strrstr(colorimetry, "jpeg") != nullptr ||
                   g_strrstr(colorimetry, "sRGB") != nullptr) {
            m_iYuvColorSpace = YUV_COLORSPACE_JPEG;
        }

        // Transfer-function detection from short tokens. GStreamer
        // ≥ 1.18 emits these alongside the matrix in caps colorimetry;
        // qtdemux for PQ HDR MP4 sends "bt2100-pq", matroskademux for
        // HDR MKV sends "smpte2084" or "arib-std-b67".
        if (g_strrstr(colorimetry, "smpte2084") != nullptr ||
            g_strrstr(colorimetry, "bt2100-pq") != nullptr ||
            g_strrstr(colorimetry, "pq")        != nullptr) {
            m_iColorTransfer = TRANSFER_PQ;
        } else if (g_strrstr(colorimetry, "arib-std-b67") != nullptr ||
                   g_strrstr(colorimetry, "bt2100-hlg")   != nullptr ||
                   g_strrstr(colorimetry, "hlg")          != nullptr) {
            m_iColorTransfer = TRANSFER_HLG;
        } else if (g_strrstr(colorimetry, "bt709") != nullptr) {
            m_iColorTransfer = TRANSFER_REC709;
        } else if (g_strrstr(colorimetry, "sRGB") != nullptr ||
                   g_strrstr(colorimetry, "srgb") != nullptr) {
            m_iColorTransfer = TRANSFER_SRGB;
        }

        // Primaries from short tokens.
        if (g_strrstr(colorimetry, "bt2020")    != nullptr ||
            g_strrstr(colorimetry, "bt2100")    != nullptr) {
            m_iColorPrimaries = PRIMARIES_REC2020;
        } else if (g_strrstr(colorimetry, "bt709") != nullptr ||
                   g_strrstr(colorimetry, "sRGB") != nullptr) {
            m_iColorPrimaries = PRIMARIES_SRGB;
        } else if (g_strrstr(colorimetry, "smpte170") != nullptr ||
                   g_strrstr(colorimetry, "bt601")    != nullptr ||
                   g_strrstr(colorimetry, "bt470")    != nullptr) {
            m_iColorPrimaries = PRIMARIES_REC601;
        } else if (g_strrstr(colorimetry, "p3")    != nullptr ||
                   g_strrstr(colorimetry, "dci")   != nullptr) {
            m_iColorPrimaries = PRIMARIES_DCI_P3;
        }

        // Range from short tokens (also overridden by the long form below).
        if (g_strrstr(colorimetry, "full") != nullptr ||
            g_strrstr(colorimetry, "jpeg") != nullptr) {
            m_iColorRange = RANGE_FULL;
        } else if (g_strrstr(colorimetry, "limited") != nullptr ||
                   g_strrstr(colorimetry, "studio")  != nullptr ||
                   g_strrstr(colorimetry, "mpeg")    != nullptr) {
            m_iColorRange = RANGE_LIMITED;
        }

        // Long form "R:M:T:P" — overrides only fields that the short
        // token parse left as AUTO. R=range  (1=limited, 2=full).
        // M=matrix (1=601, 3=709, 5/6=2020NCL/CL).
        // T=transfer (1=BT.709, 6=BT.601, 13=sRGB, 14=BT.2020-10,
        //   15=BT.2020-12, 16=SMPTE-2084 PQ, 18=ARIB-B67 HLG).
        // P=primaries (1=BT.709, 5=BT.470BG PAL, 6=SMPTE170M NTSC,
        //   9=BT.2020, 11=Display-P3, 12=DCI-P3).
        gchar** parts = g_strsplit(colorimetry, ":", -1);
        if (parts) {
            int nparts = g_strv_length(parts);
            if (nparts >= 1 && m_iColorRange == RANGE_AUTO) {
                long r = g_ascii_strtoll(parts[0], nullptr, 10);
                if      (r == 1) m_iColorRange = RANGE_LIMITED;
                else if (r == 2) m_iColorRange = RANGE_FULL;
            }
            if (nparts >= 2 && m_iYuvColorSpace == YUV_COLORSPACE_AUTO) {
                long m = g_ascii_strtoll(parts[1], nullptr, 10);
                if      (m == 1)            m_iYuvColorSpace = YUV_COLORSPACE_BT601;
                else if (m == 3)            m_iYuvColorSpace = YUV_COLORSPACE_BT709;
                else if (m == 5 || m == 6)  m_iYuvColorSpace = YUV_COLORSPACE_BT2020;
            }
            if (nparts >= 3 && m_iColorTransfer == TRANSFER_AUTO) {
                long t = g_ascii_strtoll(parts[2], nullptr, 10);
                if      (t == 1 || t == 14 || t == 15) m_iColorTransfer = TRANSFER_REC709;
                // t == 6 (BT.601/SMPTE170M) intentionally NOT mapped to
                // REC709. There is no distinct BT.601 transfer constant;
                // leaving m_iColorTransfer at its TRANSFER_AUTO default
                // lets the downstream default (SMPTE170M handling) apply
                // rather than forcing the 709 curve. Only 1/14/15 are 709.
                else if (t == 13)                      m_iColorTransfer = TRANSFER_SRGB;
                else if (t == 16)                      m_iColorTransfer = TRANSFER_PQ;
                else if (t == 18)                      m_iColorTransfer = TRANSFER_HLG;
            }
            if (nparts >= 4 && m_iColorPrimaries == PRIMARIES_AUTO) {
                long p = g_ascii_strtoll(parts[3], nullptr, 10);
                if      (p == 1)            m_iColorPrimaries = PRIMARIES_SRGB;
                else if (p == 5 || p == 6)  m_iColorPrimaries = PRIMARIES_REC601;
                else if (p == 9)            m_iColorPrimaries = PRIMARIES_REC2020;
                else if (p == 11 || p == 12) m_iColorPrimaries = PRIMARIES_DCI_P3;
            }
            g_strfreev(parts);
        }
    }

    // Mastering display peak luminance — caps "mastering-display-info"
    // ("R:G:B:WP:MIN_LUM:MAX_LUM" Bezier-style) or the simpler
    // "content-light-level" field ("MaxCLL,MaxFALL"). Either gives us
    // a hint at how to tone-map the file.
    const gchar* maxCll = gst_structure_get_string(str, "content-light-level");
    if (maxCll != nullptr) {
        long maxCllNits = g_ascii_strtoll(maxCll, nullptr, 10);
        if (maxCllNits > 0 && maxCllNits <= 10000) {
            m_fMasteringPeakNits = (float)maxCllNits;
        }
    }
