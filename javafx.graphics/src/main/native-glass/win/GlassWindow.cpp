...
#include "custom/CustomWinProc.h"
...
// skia-fx: cached bridge to QuantumToolkit.firePulseFromNativeTimer().
// Used by the modal-loop pulse-injection WM_TIMER handler so JFX
// pulses survive Win32's nested message loop during title-bar drag
// / caption-button hold. Resolved lazily on first WM_TIMER firing
// (we can't do it from initIDs without plumbing a fresh env); null
// after a failed resolve, in which case the WM_TIMER handler is a
// graceful no-op. Class is held as a global ref so it survives
// across the AttachCurrentThread cycle inside JNI helpers.
static jclass    sQuantumToolkitClass     = NULL;
static jmethodID sQuantumFirePulseMID     = NULL;
static bool      sQuantumFirePulseTried   = false;

static void GlassWindow_firePulseFromTimer(JNIEnv *env)
{
    if (!env) return;
    if (!sQuantumFirePulseTried) {
        sQuantumFirePulseTried = true;
        jclass localCls = env->FindClass("com/sun/javafx/tk/quantum/QuantumToolkit");
        if (env->ExceptionCheck()) { env->ExceptionClear(); return; }
        if (!localCls) return;
        sQuantumToolkitClass = (jclass) env->NewGlobalRef(localCls);
        env->DeleteLocalRef(localCls);
        if (!sQuantumToolkitClass) return;
        sQuantumFirePulseMID = env->GetStaticMethodID(
            sQuantumToolkitClass, "firePulseFromNativeTimer", "()V");
        if (env->ExceptionCheck()) {
            env->ExceptionClear();
            env->DeleteGlobalRef(sQuantumToolkitClass);
            sQuantumToolkitClass = NULL;
            sQuantumFirePulseMID = NULL;
            return;
        }
    }
    if (sQuantumToolkitClass && sQuantumFirePulseMID) {
        env->CallStaticVoidMethod(sQuantumToolkitClass, sQuantumFirePulseMID);
...
//        fflush(stdout);
    ...
    switch (msg) {
        ...
        case WM_SIZING:
            ...
            break;
        case WM_SIZE: {
            // The earlier secondary-HGLRC slate-900 clear was needed
            // when each WM_SIZE forced a destroy+recreate of the Skia
            // direct-present surface — the recreate was slow enough
            // that DWM could compose a stretched stale buffer before
            // the next SwapBuffers. With in-place GL/D3D resize (see
            // SkiaPresentable.lockResources + openjfx_skia_surface_resize_gl),
            // the WM_SIZE → liveRepaintRenderJob chain completes
            // inside the WndProc call, so DWM composes the freshly
            // painted frame directly. Removing the cross-thread aux-
            // HGLRC clear also eliminates the HDC contention that was
            // contributing to the ScrollPane shrink/expand shimmer
...
//            }
        ...
        case WM_ERASEBKGND:
            // skia-fx: suppress the class-brush background erase.
            //
            // WindowStage pushes the Scene's fill colour (default
            // Color.WHITE) into GCLP_HBRBACKGROUND via
            // _setBackground2. Stock JFX lets DefWindowProc paint
            // that brush on every WM_ERASEBKGND — including the
            // implicit ones DWM issues during a window's first show.
            // The painted brush ends up in the redirection bitmap
            // and reads as a visible solid-colour flash before the
            // JFX render thread's first SwapBuffers/Present lands.
            //
            // Returning 1 tells Windows "background is already
            // erased" so the brush is never painted. The Skia swap
            // chain Present is the sole source of pixels visible in
            // the client area, which is what skia-fx wants anyway.
            //
            // This is Microsoft's documented pattern for windows
            // that paint their own content (see
            // docs/WINDOW_FLICKER_INVESTIGATION.md for refs).
            //
            // Safe for steady state: PresentingPainter forces
            // renderEverything every frame (after the
            // freshBackBuffer-after-present fix), so the back
            // buffer never needs the OS-painted brush as fallback.
            return 1;
        case WM_PAINT:
        ...
        case WM_MOUSELEAVE:
        case WM_MOUSEMOVE:
            // skia-fx: caption-button bypass cancellation path.
            // User pressed an OS caption button, then dragged into
            // the client area before releasing. WM_LBUTTONUP fires
            // (instead of WM_NCLBUTTONUP) — treat as cancel and
            // swallow the click so JFX doesn't see a phantom click
            // on whichever client node is under the cursor.
            if (msg == WM_LBUTTONUP && m_pendingCaptionButton != 0) {
                m_pendingCaptionButton = 0;
                return 0;
            }
            if (!IsEnabled()) {
                HandleFocusDisabledEvent();
        ...
        case WM_CAPTURECHANGED:
            // skia-fx: capture stolen externally (focus loss, child
            // dialog, etc.) — drop pending caption-button state.
            // Don't fire the action; user didn't release inside.
            m_pendingCaptionButton = 0;
                ...
                if (wParam == HTMINBUTTON || wParam == HTMAXBUTTON || wParam == HTCLOSE) {
                    return 0;
                       ...
                       (wParam == HTMINBUTTON || wParam == HTMAXBUTTON ||
                        wParam == HTCLOSE)) {
                // skia-fx: bypass DefWindowProc's caption-button
                // tracking loop (it suspends DWM composition for the
                // window's duration). Remember which button; the
                // matching WM_NCLBUTTONUP posts the WM_SYSCOMMAND.
                // No SetCapture — JFX manages its own capture and
                // racing with it cleared the pending state mid-click.
                m_pendingCaptionButton = (UINT) wParam;
                return 0;
            }
        ...
        case WM_NCMOUSEMOVE:
            // skia-fx: complete the caption-button bypass on
            // release. wParam is the OS-classified HT code; we fire
            // the action only if it matches the press.
            if (msg == WM_NCLBUTTONUP && m_pendingCaptionButton != 0) {
                HandleCaptionButtonRelease(wParam);
                return 0;
            }
            if (m_isExtended) {
                HandleNonClientMouseEvents(msg, wParam, lParam);
        ...
        case WM_TOUCH:
            ...
            break;
        case WM_TIMER:
            // skia-fx: modal-loop pulse heartbeat. Bracketed by
            // WM_ENTERSIZEMOVE / WM_EXITSIZEMOVE below, plus the
            // caption-button bracket inside WM_NCLBUTTONDOWN for
            // non-extended decorated windows. Re-enters
            // QuantumToolkit.postPulse() so JFX animations / video
            // keep advancing while DefWindowProc's nested message
            // loop holds the Glass main thread.
            if (wParam == IDT_GLASS_PULSE_MODAL) {
                GlassWindow_firePulseFromTimer(GetEnv());
                return 0;
            }
            HandleViewTimerEvent(GetHWND(), wParam);
            return 0;
        case WM_ENTERSIZEMOVE:
            // Pulse once at the start of the modal sizing/moving
            // loop, then run a heartbeat WM_TIMER so JFX animations
            // keep advancing while DefWindowProc holds the message
            // thread inside its pump. NOTE: DefWindowProc may
            // additionally block the message thread for up to
            // GetDoubleClickTime() ms BEFORE firing WM_ENTERSIZEMOVE
            // (waiting to see whether a drag is starting), and no
            // injected timer or message can run during that
            // pre-loop wait. The residual click-without-drag pause
            // on the title bar is a Win32 limitation we accept in
            // exchange for keeping native drag features (Aero Snap,
            // snap layouts, DWM drag-shadow).
            GlassWindow_firePulseFromTimer(GetEnv());
            ::SetTimer(GetHWND(), IDT_GLASS_PULSE_MODAL, 16, NULL);
            break;
        case WM_EXITSIZEMOVE:
            ::KillTimer(GetHWND(), IDT_GLASS_PULSE_MODAL);
            // One pulse at loop end so the render resumes
            // immediately instead of waiting for the next normal
            // master-timer tick.
            GlassWindow_firePulseFromTimer(GetEnv());
            break;
        case WM_GETOBJECT: {
...
void GlassWindow::HandleCaptionButtonRelease(WPARAM hitOnRelease)
{
    const UINT btn = m_pendingCaptionButton;
    m_pendingCaptionButton = 0;

    // Fire only if the release HT matches the press — i.e. the user
    // released over the same OS caption button they pressed. Any
    // other HT (caption, edge, another button) is treated as
    // cancellation, matching the OS modal loop's behaviour.
    if (hitOnRelease != (WPARAM) btn) {
        return;
    }
    ...
    switch (btn) {
        case HTMINBUTTON:
            ...
            break;
        case HTMAXBUTTON:
            sc = ::IsZoomed(GetHWND()) ? SC_RESTORE : SC_MAXIMIZE;
            break;
        case HTCLOSE:
            sc = SC_CLOSE;
            break;
    ...
    if (sc != 0) {
        ::PostMessage(GetHWND(), WM_SYSCOMMAND, sc, 0);
    }
}
...
void GlassWindow::HandleDPIEvent(WPARAM wParam, LPARAM lParam)
{
    ...
    // Per-Monitor-DPI-Aware-V2 contract: on a DPI change the OS does NOT auto-
    // scale our window (that is the whole point of V2 — crisp, not bitmap-
    // stretched), so resizing the window to the DPI-correct geometry is OUR
    // responsibility. lParam is the OS-suggested RECT, sized so the window's
    // LOGICAL size stays constant (device px grows ~scale×) — i.e. UI elements
    // keep the same physical size and just render with more pixels.
    //
    // We must do this here because a pure DPI change fires Glass RESCALE (scale
    // only), which never recomputes the Scene's logical size. The SetWindowPos
    // resize fires WM_SIZE -> RESIZE -> changedSize(devicePx / scale), which is
    // what re-lays-out the scene to FILL the window on a monitor move. Without
    // it the content rendered crisp but filled only part of the window.
    //
    // Order matters: notifyScaleChanged first (so the new scale is in effect),
    // then resize (so the WM_SIZE -> changedSize divides device px by the new
    // scale). Windows-only path; macOS/Linux Glass are unaffected.
    //
    // SWP_FRAMECHANGED makes this work for EVERY stage type, not just decorated:
    // a CUSTOM (frameless / StageStyle.UNDECORATED|TRANSPARENT) window defines its
    // client area via WM_NCCALCSIZE. On a DPI change the frame metrics change, but
    // without SWP_FRAMECHANGED the OS does not re-send WM_NCCALCSIZE, so the client
    // rect (hence the size Glass reports to the Scene) is computed against stale
    // frame assumptions and the scene lays out smaller than the window — it fills
    // only part until a manual resize forces the recompute. SWP_FRAMECHANGED forces
    // WM_NCCALCSIZE + WM_SIZE for all window types; for a decorated window the frame
    // genuinely changed with the DPI, so recomputing it is correct (no regression) —
    // it just guarantees the same RESIZE -> relayout that decorated windows already
    // get. Position/size set and the scale call above are unchanged.
    ...
    (JNIEnv *env, jclass cls)
{
     ...
     // skia-fx: cache the WinWindow.jniHitTest jmethodID for the
     // custom-decorations WindowProc. Optional — windows that aren't
     // custom-decorated never invoke it.
     SkiaCustomWinProc::initIDs(env, cls);
    ...
    (JNIEnv *env, jobject jThis, jlong ownerPtr, jlong screenPtr, jint mask)
{
    ENTER_MAIN_THREAD_AND_RETURN(jlong)
    {
        ...
        // skia-fx: StageStyle.CUSTOM — give the
        // window the full overlapped style (caption + thick frame +
        // sizebox + min/max boxes) so the OS still does Aero snap,
        // Win11 snap-layouts, edge resize, taskbar thumbnail, and
        // window animations. After window creation we subclass the
        // WindowProc via SkiaCustomWinProc::install() (see custom/),
        // which owns WM_NCCALCSIZE (removes the visible frame +
        // caption) and WM_NCHITTEST (routes to the Java-side region
        // snapshot).
        const bool isCustomDecorations =
            (mask & com_sun_glass_ui_Window_CUSTOM_DECORATIONS) != 0;
        if (isCustomDecorations) {
            dwStyle = WS_CLIPCHILDREN | WS_OVERLAPPEDWINDOW;
        ...
        if (hWnd && isCustomDecorations) {
            // skia-fx: subclass the WindowProc with the custom-
            // decorations handler. This installs WM_NCCALCSIZE +
            // WM_NCHITTEST + the rest of the nfx-lib ported pipeline
            // for this HWND. See win/custom/CustomWinProc.cpp.
            SkiaCustomWinProc::install(GetEnv(), hWnd, jThis);

            // Apply Win11 rounded corners (DWMWA_WINDOW_CORNER_PREFERENCE
            // = 33, DWMWCP_ROUND = 2). No-op on Windows versions that
            // don't recognise the attribute.
...
// ---- skia-fx DWM accent setters for StageStyle.CUSTOM windows ---------
//
// argb=-1 (0xFFFFFFFF) is treated as "reset to default" — matches
// DWMWA_COLOR_DEFAULT in dwmapi.h. For colours we use COLORREF
// (0x00BBGGRR) — DWM takes the bottom 24 bits as a Windows colour ref.

namespace {
    // DWMWA_* attribute integer values (matches dwmapi.h Win10+).
    // The Win10 SDK is in scope at build time so we can use the
    // symbolic constants directly; the integer values are inlined
    // here for reference. DWMWA_COLOR_DEFAULT = 0xFFFFFFFF means
    // "reset to platform default" for the colour attributes.

    // Convert a packed ARGB int from Java to a COLORREF (0x00BBGGRR).
    // Java passes -1 to mean "default" — we pass that through as
    // DWMWA_COLOR_DEFAULT (= 0xFFFFFFFF).
    DWORD argbToDwmColor(jint argb) {
        if (argb == -1) return 0xFFFFFFFF; // DWMWA_COLOR_DEFAULT
...
/*
 * Class:     com_sun_glass_ui_win_WinWindow
 * Method:    _setDwmCaptionColor
 * Signature: (JI)V
 */
JNIEXPORT void JNICALL Java_com_sun_glass_ui_win_WinWindow__1setDwmCaptionColor
    (JNIEnv *env, jobject jThis, jlong ptr, jint argb)
{
    ENTER_MAIN_THREAD()
    {
        DWORD c = color;
    ...
    ARG(color) = argbToDwmColor(argb);
    PERFORM();
}

/*
 * Class:     com_sun_glass_ui_win_WinWindow
 * Method:    _setDwmTextColor
 * Signature: (JI)V
 */
JNIEXPORT void JNICALL Java_com_sun_glass_ui_win_WinWindow__1setDwmTextColor
    (JNIEnv *env, jobject jThis, jlong ptr, jint argb)
{
    ENTER_MAIN_THREAD()
    {
        DWORD c = color;
        ::DwmSetWindowAttribute(hWnd, DWMWA_TEXT_COLOR, &c, sizeof(c));
    }
    DWORD color;
    LEAVE_MAIN_THREAD_WITH_hWnd;
...
/*
 * Class:     com_sun_glass_ui_win_WinWindow
 * Method:    _setDwmBorderColor
 * Signature: (JI)V
 */
JNIEXPORT void JNICALL Java_com_sun_glass_ui_win_WinWindow__1setDwmBorderColor
    (JNIEnv *env, jobject jThis, jlong ptr, jint argb)
{
    ENTER_MAIN_THREAD()
    {
...
/*
 * Class:     com_sun_glass_ui_win_WinWindow
 * Method:    _setDwmCornerPreference
 * Signature: (JI)V
 *
 * pref: 0 = DEFAULT, 1 = DONOTROUND, 2 = ROUND, 3 = ROUNDSMALL
 *       (matches DWM_WINDOW_CORNER_PREFERENCE values).
 */
JNIEXPORT void JNICALL Java_com_sun_glass_ui_win_WinWindow__1setDwmCornerPreference
    (JNIEnv *env, jobject jThis, jlong ptr, jint pref)
{
    ENTER_MAIN_THREAD()
    {
        DWORD p = (DWORD) preference;
