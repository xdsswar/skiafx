...
private:
    ...
    // skia-fx: which OS caption button (HTMINBUTTON / HTMAXBUTTON /
    // HTCLOSE) is currently being held, or 0 if none. Set on
    // WM_NCLBUTTONDOWN when we bypass DefWindowProc's caption-button
    // tracking modal loop (which otherwise freezes the window's
    // composition for the duration of the press). Consumed on
    // WM_NCLBUTTONUP / WM_LBUTTONUP — if the cursor is still over
    // the same button on release we post the corresponding
    // WM_SYSCOMMAND. Cleared on WM_CAPTURECHANGED so we don't act
    // on stale state if capture is yanked.
    UINT m_pendingCaptionButton = 0;

    /**
     * skia-fx: dispatch a WM_SYSCOMMAND for {@link
     * #m_pendingCaptionButton} if the release happened over the
     * same OS caption button as the press. Always clears the pending
     * state, even if the action doesn't fire. Called from the
     * WM_NCLBUTTONUP case; {@code hitOnRelease} is the HT code the
     * OS classified the release at.
     */
    void HandleCaptionButtonRelease(WPARAM hitOnRelease);
