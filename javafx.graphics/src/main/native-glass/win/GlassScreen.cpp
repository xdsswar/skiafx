...
void GlassScreen::LoadDPIFuncs(jint awareRequested)
{
    ...
    // Prefer Per-Monitor-DPI-Aware-V2 (Win10 1607+): it scales the non-client
    // area, dialogs, and gives correct GetDpiForWindow per monitor — the modern
    // contract that keeps content crisp on mixed-DPI multi-monitor setups and
    // adapts on monitor moves. The DPI_AWARENESS_CONTEXT values aren't in our
    // Win7 SDK headers; PER_MONITOR_AWARE_V2 == (DPI_AWARENESS_CONTEXT)-4.
    // Loaded dynamically; falls back to the legacy SHCore v1 path below.
    bool v2Set = false;
    {
        typedef BOOL (WINAPI *FnSetProcessDpiAwarenessContext)(HANDLE);
        HMODULE hUser32 = ::GetModuleHandleW(L"user32.dll");
        if (hUser32) {
            FnSetProcessDpiAwarenessContext pSetCtx =
                (FnSetProcessDpiAwarenessContext) ::GetProcAddress(
                    hUser32, "SetProcessDpiAwarenessContext");
            if (pSetCtx) {
                v2Set = pSetCtx((HANDLE)(INT_PTR)-4) != FALSE;
            }
        }
    }
    (void) awareRequested;
    if (v2Set) {
        // Per-Monitor-V2 active — skip the legacy fallbacks.
