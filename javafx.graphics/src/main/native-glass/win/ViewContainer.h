...
class ViewContainer {
    ...
    public:
        enum {
            ...
            // skia-fx: heartbeat timer that drives JFX pulses from
            // inside Win32 modal sizing/moving loops. The OS pump
            // inside DefWindowProc's modal loop dispatches WM_TIMER,
            // but the normal pulse path (Glass's main message loop)
            // is stalled. Set in WM_ENTERSIZEMOVE, killed in
            // WM_EXITSIZEMOVE. See QuantumToolkit.firePulseFromNativeTimer.
            IDT_GLASS_PULSE_MODAL,
        };
