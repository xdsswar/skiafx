        ...
        LPCTSTR lpWindowName, DWORD dwExStyle, DWORD dwStyle, HBRUSH hbrBackground)
{
    ...
    // CS_OWNDC: stable per-window HDC so the Skia GL pipeline can hold
    // it across wglMakeCurrent / SwapBuffers calls. Safe for the GDI
    // BitBlt path (SetDIBitsToDevice doesn't care about pixel format).
