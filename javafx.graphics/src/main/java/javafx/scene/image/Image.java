...
public class Image {
    ...
    // Package-private (was private) so vector subclasses (SvgImage) can invoke
    // the master constructor directly — it sets fields WITHOUT calling
    // initialize(), so no raster PlatformImage is loaded. The 7-arg signature
    // is unique, so this introduces no public API and no overload ambiguity.
    Image(String url, InputStream is,
    ...
    /**
     * Publishes an intrinsic (vector) size for a vector-backed subclass. The
     * width/height become observable via {@link #widthProperty()} /
     * {@link #heightProperty()} and drive layout the same way a raster image's
     * pixel dimensions do, but no {@code PlatformImage} is involved. Marks the
     * image as fully "loaded" (progress = 1) so downstream consumers treat it
     * as ready.
     */
    final void setVectorSize(double w, double h) {
        storePlatformImageWH(null, w, h);
    ...
    /**
     * Reports a load failure for a vector-backed subclass, mirroring the
     * raster path's error handling so {@link #errorProperty()} /
     * {@link #exceptionProperty()} behave consistently.
     */
    final void setVectorError(Exception e) {
        setException(e);
        setError(true);
        setProgress(1);
    }
    ...
    // Package-private so vector subclasses (SvgImage) resolve URL strings with
    // exactly the same semantics (classpath resource / file / URL / data URI).
