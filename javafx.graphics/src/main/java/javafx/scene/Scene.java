...
//        System.err.println("Schedule a snapshot in the future");
    ...
    class ScenePulseListener implements TKPulseListener {
        ...
        public void pulse() {
            ...
            // skia-fx: settle CSS for nodes created DURING the layout pass.
            // VirtualFlow recycles list cells inside layoutChildren, and a cell
            // factory that builds new CSS-styled nodes in updateItem adds them
            // AFTER the CSS pass above already ran. Left unsettled, those nodes
            // render with default styling for one frame — a Shape's default
            // BLACK fill, unlaid-out (invisible) text — i.e. the "avatar flashes
            // black while scrolling a ListView" bug. The root reports pending
            // CSS (cssFlag != CLEAN) when this happens, so re-run CSS + layout
            // until it settles, bounded so a pathological never-converging tree
            // can never spin the pulse (worst case falls back to today's
            // one-frame lag). Zero cost on static scenes: the loop body is
            // skipped whenever layout added no CSS-pending nodes.
            {
