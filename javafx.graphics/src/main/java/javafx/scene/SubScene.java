...
import com.sun.javafx.scene3d.Scene3D; // skia-fx: runtime anti-aliasing control (internal)
...
public non-sealed class SubScene extends Node {
    ...
    // ------------------------------------------------------------------------
    //  skia-fx extension: runtime anti-aliasing control.
    //
    //  Stock JavaFX fixes SceneAntialiasing (above) at construction. The skia-fx
    //  Skia/bgfx 3D pipeline renders MSAA inside its GPU target and can change the
    //  sample count at any time, so these additive methods let an application toggle,
    //  or pick the level of, anti-aliasing for this SubScene at RUNTIME. They delegate
    //  to the internal (non-exported) Scene3D control and are no-ops on a non-3D
    //  pipeline / before the SubScene is shown. Existing JavaFX code is unaffected.
    // ------------------------------------------------------------------------

    // One Scene3D control per SubScene, created lazily on first use and reused by all
    // the runtime AA methods (it is bound to this SubScene, so one instance suffices).
    ...
    /**
     * Enables or disables anti-aliasing for this {@code SubScene} at runtime
     * (skia-fx extension). Unlike the constructor-only
     * {@link #getAntiAliasing() SceneAntialiasing}, this can be changed at any time;
     * it takes effect on the next rendered frame. Call on the FX thread after the
     * SubScene is shown. No-op if the active pipeline lacks runtime 3D MSAA.
     *
     * @param on {@code true} to anti-alias, {@code false} to disable
     */
    public final void setAntiAliasing(boolean on) {
        scene3D().setAntiAliasing(on);
    }

    /**
     * Returns whether runtime anti-aliasing is currently enabled for this
     * {@code SubScene} (skia-fx extension).
     *
     * @return {@code true} if anti-aliasing is on
     */
    public final boolean isAntiAliasingEnabled() {
        return scene3D().isAntiAliasing();
    }

    /**
     * Sets the exact MSAA sample count for this {@code SubScene} at runtime
     * (skia-fx extension): {@code 1} = off, or {@code 2}, {@code 4}, {@code 8}
     * (snapped down to GPU support). Takes effect on the next rendered frame.
     *
     * @param samples the desired sample count
     */
    public final void setAntiAliasingSampleCount(int samples) {
        scene3D().setSampleCount(samples);
    }
