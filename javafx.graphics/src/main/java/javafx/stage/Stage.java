...
import com.sun.javafx.stage.custom.CustomDecorations;
...
public class Stage extends Window {
    ...
    static {
        StageHelper.setStageAccessor(new StageHelper.StageAccessor() {
            ...
            @Override
            public CustomDecorations getCustomDecorations(Stage stage) {
                return stage.getCustomDecorations();
            }
        });
    }
    ...
    // ---- Custom decorations (StageStyle.CUSTOM) -----------------
    //
    // Per-stage holder for application-declared hit regions (caption,
    // close, max, min, full-screen, system menu) and DWM accent state
    // (title bar / text / border colours, corner preference). The
    // native side reads a snapshot of the hit regions on every
    // WM_NCHITTEST; FX-thread mutations publish a new immutable
    // snapshot atomically.
    //
    // For stages whose style is NOT CUSTOM, the
    // setters below still store the values (silent no-op policy) so
    // unconditional configuration code keeps working without
    // exceptions — the values are simply never consulted by the
    // native side.

    private CustomDecorations customDecorations;

    private CustomDecorations decorations() {
        if (customDecorations == null) {
            customDecorations = new CustomDecorations();
        }
        return customDecorations;
    }

    /**
     * Declares one or more {@link Region}s as draggable caption (title-bar)
     * areas on a custom-decorated stage. While the cursor is over any of
     * these regions, the system treats it as a window-drag handle: a
     * primary click + drag moves the stage, a double-click toggles
     * maximize, and a secondary click opens the platform system menu.
     * Each declared region also receives the {@code :ht-caption} CSS
     * pseudo-class while hovered, so it can be styled in CSS.
     *
     * <p>Buttons declared via {@link #setCloseRegion(Region)},
     * {@link #setMaxRegion(Region)}, {@link #setMinRegion(Region)},
     * {@link #setFullScreenRegion(Region)} and
     * {@link #setSystemMenuRegion(Region)} take precedence over caption
     * regions when their bounds overlap — useful when buttons live
     * inside the caption bar.</p>
     *
     * <p>Has no effect on stages whose style is not
     * {@link StageStyle#CUSTOM};
     * the regions are stored but never consulted.</p>
     *
     * @param regions the regions to mark as caption / drag areas,
     *                or none to clear
     * @since 25
     */
    public final void setCaptionRegions(Region... regions) {
        decorations().setCaptionRegions(regions);
    }

    /**
     * Declares the {@link Region} that acts as the close button on a
     * custom-decorated stage. Clicking it sends the platform-native
     * close request to the stage (same as the platform-provided close
     * button on a {@link StageStyle#DECORATED} stage). While the
     * cursor is over this region, the node receives the
     * {@code :ht-close} CSS pseudo-class.
     *
     * <p>Has no effect on stages whose style is not
     * {@link StageStyle#CUSTOM}.</p>
     *
     * @param region the close-button region, or {@code null} to clear
     * @since 25
     */
    public final void setCloseRegion(Region region) {
        decorations().setCloseRegion(region);
    }

    /**
     * Declares the {@link Region} that acts as the maximize button on
     * a custom-decorated stage. On Windows&nbsp;11, hovering this
     * region also triggers the snap-layouts flyout. While hovered, the
     * node receives the {@code :ht-max} CSS pseudo-class.
     *
     * <p>Has no effect on stages whose style is not
     * {@link StageStyle#CUSTOM}.</p>
     *
     * @param region the maximize-button region, or {@code null} to clear
     * @since 25
     */
    public final void setMaxRegion(Region region) {
        decorations().setMaxRegion(region);
    }

    /**
     * Declares the {@link Region} that acts as the minimize / iconify
     * button on a custom-decorated stage. While hovered, the node
     * receives the {@code :ht-min} CSS pseudo-class.
     *
     * <p>Has no effect on stages whose style is not
     * {@link StageStyle#CUSTOM}.</p>
     *
     * @param region the minimize-button region, or {@code null} to clear
     * @since 25
     */
    public final void setMinRegion(Region region) {
        decorations().setMinRegion(region);
    }

    /**
     * Declares the {@link Region} that acts as the full-screen toggle
     * button on a custom-decorated stage. The application is
     * responsible for actually toggling {@link #setFullScreen(boolean)}
     * — this method only governs the hit-test routing and the
     * {@code :ht-fscreen} CSS pseudo-class for hover styling.
     *
     * <p>Has no effect on stages whose style is not
     * {@link StageStyle#CUSTOM}.</p>
     *
     * @param region the full-screen-button region, or {@code null} to clear
     * @since 25
     */
    public final void setFullScreenRegion(Region region) {
        decorations().setFullScreenRegion(region);
    }

    /**
     * Declares the {@link Region} that acts as the system-menu hot zone
     * (typically a small icon at the top-left of a custom title bar)
     * on a custom-decorated stage. A primary click opens the platform
     * system menu; secondary click on any caption region also opens it.
     *
     * <p>Has no effect on stages whose style is not
     * {@link StageStyle#CUSTOM}.</p>
     *
     * @param region the system-menu region, or {@code null} to clear
     * @since 25
     */
    public final void setSystemMenuRegion(Region region) {
        decorations().setSystemMenuRegion(region);
    }

    /**
     * Sets the title-bar accent colour for the platform-painted
     * window frame on a custom-decorated stage. On Windows this maps
     * to {@code DWMWA_CAPTION_COLOR}; on platforms without an
     * equivalent attribute this is stored but never applied.
     *
     * @param color the accent colour, or {@code null} to fall back to the platform default
     * @since 25
     */
    public final void setTitleBarColor(Color color) {
        decorations().setTitleBarColor(color);
    }

    /**
     * Sets the caption-text colour for the platform-painted window
     * frame on a custom-decorated stage. On Windows this maps to
     * {@code DWMWA_TEXT_COLOR}.
     *
     * @param color the caption-text colour, or {@code null} to fall back to the platform default
     * @since 25
     */
    public final void setCaptionTextColor(Color color) {
        decorations().setCaptionTextColor(color);
    }

    /**
     * Sets the window-border colour for the platform-painted window
     * frame on a custom-decorated stage. On Windows this maps to
     * {@code DWMWA_BORDER_COLOR}.
     *
     * @param color the border colour, or {@code null} to fall back to the platform default
     * @since 25
     */
    public final void setBorderColor(Color color) {
        decorations().setBorderColor(color);
    }

    /**
     * Sets the corner-preference style for a custom-decorated stage.
     * Honoured on Windows&nbsp;11 via
     * {@code DWMWA_WINDOW_CORNER_PREFERENCE}; no-op elsewhere.
     *
     * @param preference the corner preference; {@code null} is treated as
     *                   {@link WindowCornerPreference#DEFAULT}
     * @since 25
     */
    public final void setCornerPreference(WindowCornerPreference preference) {
        decorations().setCornerPreference(preference);
    }

    /**
     * Internal accessor for the custom-decorations holder, used by the
     * Quantum / Glass peer to read the hit-region snapshot and DWM
     * accent state when the stage's style is
     * {@link StageStyle#CUSTOM}.
     */
    final CustomDecorations getCustomDecorations() {
        return customDecorations;
    }
