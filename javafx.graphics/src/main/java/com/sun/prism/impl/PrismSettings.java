...
public final class PrismSettings {
    ...
    static {
         ...
         *
         * openjfx-skia: default ON. Safe because the Skia GL present
         * path also defaults to offscreen-FBO mode, which preserves
         * the back-buffer's content across frames — so painting only
         * the dirty regions leaves the rest visually intact. If the
         * offscreen-FBO path is disabled (OPENJFX_SKIA_GL_OFFSCREEN=0),
         * SwapBuffers leaves post-swap content undefined and dirty
         * regions produce distortion / flicker, so also pass
         * -Dprism.dirtyopts=false in that configuration.
         */
        ...
        // openjfx-skia: Skia is the *only* rendering pipeline. The
        // Prism-era backends (d3d/es2/mtl/sw/j2d) have been removed
        // from the tree — their Skia equivalents (Ganesh GL,
        // D3D12 swap-chain, CPU SkSurface raster) live inside
        // SkiaPresentable's four-tier allocation. The prism.order
        // system property is honoured as an override only for
        // diagnostics (e.g. setting it to a bogus value will hard-fail
        // at GraphicsPipeline.createPipeline below — that is the
        // documented behaviour).
        String[] tryOrderArr = new String[] { "skia" };
        ...
        if (verbose) {
            System.out.print("Skia pipeline init order: ");
