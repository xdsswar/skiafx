                            ...
                            Effect defaultInput)
    {
        ...
        if (botimg != null) {
            if (!botimg.validate(fctx)) {
                // The bottom layer (e.g. a DropShadow's blurred shadow) could
                // not be produced this pulse — its offscreen image came back
                // null/incompatible. Degrade gracefully: drop the bottom layer
                // and render the top (content) layer alone instead of returning
                // an invalid result. The old (null,null) return made
                // PrEffectHelper re-render the whole subtree maxValidateRetries
                // times and then drop the decorated node entirely — the
                // "giving up on DropShadow" stutter. Errors degrade, never abort
                // the frame. unref() releases botimg (no-op if its image is null).
                botimg.unref();
                botimg = null;
            } else if (renderHelper instanceof ImageDataRenderer) {
        ...
        if (!topimg.validate(fctx)) {
            // Top (content) layer invalid. Don't leak botimg and don't abort:
            // fall back to the (validated) bottom layer alone. Rare — content
            // is the direct node render; if it failed the shadow usually did
            // too and we already took the content-only path above.
            topimg.unref();
            return botimg;
        }
