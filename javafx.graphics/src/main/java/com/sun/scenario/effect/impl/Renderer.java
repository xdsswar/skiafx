        ...
        if (r == null) {
            ...
            for (Iterator<Renderer> iter = renderers.iterator(); iter.hasNext();)
            {
                ...
                if (ren.getRendererState() == RendererState.DISPOSED) {
                    // Release each cached effect peer's native resources before
                    // dropping the renderer. peerCache is otherwise never reaped,
                    // so peers holding native handles (e.g. the Skia peers'
                    // cached SkImage/SkImageFilter) would leak whenever a renderer
                    // is disposed (display/device change). Copy first — dispose()
                    // may mutate, and the view is over a synchronized map.
                    for (EffectPeer peer : new ArrayList<>(ren.getPeers())) {
                        try {
                            peer.dispose();
                        } catch (Throwable ignore) {
                            // best-effort cleanup; never block renderer reaping
                        }
                    }
