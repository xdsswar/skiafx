...
public class PerspectiveTransformState {
    ...
    /** The forward 3x3 perspective matrix (source -> dest), as last passed to
     *  {@link #updateTx}. The Skia MatrixTransform filter needs the forward
     *  matrix; {@link #getITX} returns the inverse used for sampling. */
    public float[][] getTx() {
        return tx;
    }

    public void updateTx(float[][] tx) {
        // Retain the forward matrix (callers only kept the inverse before).
        for (int i = 0; i < 3; i++) {
            System.arraycopy(tx[i], 0, this.tx[i], 0, 3);
        }
