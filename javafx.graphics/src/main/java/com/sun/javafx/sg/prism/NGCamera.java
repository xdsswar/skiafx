...
public abstract class NGCamera extends NGNode {
    ...
    @Override
    public void release() {
        // skia-fx: nothing to release. The upstream TODO referred to Prism's native
        // (D3D/ES2) camera state; the Skia/bgfx pipeline keeps the camera entirely
        // Java-side (projViewTx is computed here, no native handle), so there are no
        // native camera resources to free.
    }
