...
public class CacheFilter {
    ...
    // Device-space scissor for the scroll dirty rect on the Skia HiDPI cache
    // path. Separate from TEMP_RECT because dirtyBounds aliases TEMP_RECT. (NEW-H1)
    private static final Rectangle TEMP_DIRTY_DEVICE = new Rectangle();
    ...
    // skia-fx HiDPI: extra pixel scale baked into the cache TEXTURE resolution
    // (not the cache transform) so a logical-xform cache is still device-res and
    // composites crisp. 1.0 except for the non-scaleHint Skia path on HiDPI.
    private float cachePixelScaleX = 1.0f;
    private float cachePixelScaleY = 1.0f;
    // The raw Graphics pixel scale the current cache was built at; if it changes
    // (window moved to a different-DPI monitor) the cache must rebuild at the new
    // device resolution or it stays blurry.
    private float builtPixelScaleX = -1.0f;
    private float builtPixelScaleY = -1.0f;
                                      ...
                                      float pixelScaleX, float pixelScaleY)
    {
        ...
        // Window moved to a different-DPI monitor → rebuild at the new device
        // resolution (otherwise the old, now-wrong-scale cache stays on screen
        // blurry). builtPixelScaleX is -1 before the first build.
        if (builtPixelScaleX != pixelScaleX || builtPixelScaleY != pixelScaleY) {
            return true;
        }
    ...
    void updateScreenXform(double[] xformInfo) {
        ...
        if (scaleHint) {
            ...
            // screenScale is the ratio between the current render scale and the
            // scale the cache was built at. It is 1.0 (identity) for a normal
            // logical cache, and 1/pixelScale for a skia-fx HiDPI DEVICE-scaled
            // cache (cachedScale = xformScale*pixelScale) so the device-res cache
            // composites 1:1 — uniform, so cache reuse restores the right scale.
            double screenScaleX = xformInfo[0] / cachedScaleX;
            double screenScaleY = xformInfo[1] / cachedScaleY;
            screenXform.setToScale(screenScaleX, screenScaleY);
            if (rotateHint) {
                screenXform.rotate(xformInfo[2] - cachedRotate);
            }
        }
    }
    ...
    void render(Graphics g) {
        ...
        if (needToRenderCache(xform, xformInfo, pixelScaleX, pixelScaleY)) {
            ...
            if (scaleHint) {
                ...
                cachedRotate = 0;
                // The Y diagonal must use cachedScaleY, not cachedScaleX —
                // feeding scaleX into the d element shrinks/stretches the cached
                // node vertically under anisotropic (non-square) DPI. Upstream
                // uses a single uniform cachedScale; this fork split X/Y (line
                // above + updateScreenXform's separate divides) but left the
                // Y term wired to scaleX. (bugs.md H3)
            ...
            // transform, so caching at the (logical) cachedXform builds a logical-
            // resolution cache that is then upscaled on composite -> blurry cached
            // regions on 150%/175% monitors. Keep cachedXform logical (so the
            // cheap translation-only cache reuse keeps working) but render the
            // cache at DEVICE resolution: createImageData allocates the texture
            // at bounds×cachePixelScale and renderNodeToCache sets that scale on
            // the cache Graphics. screenXform stays as set above; the device-res
            // cache composites 1:1 once the window Graphics re-applies the scale.
            // scaleHint already builds a device-resolution cache via cachedScale,
            // so cachePixelScale is left at 1 there (no double scaling).
            cachePixelScaleX = (!scaleHint && (g instanceof SkiaGraphics)) ? pixelScaleX : 1.0f;
            cachePixelScaleY = (!scaleHint && (g instanceof SkiaGraphics)) ? pixelScaleY : 1.0f;
            builtPixelScaleX = pixelScaleX;
            builtPixelScaleY = pixelScaleY;
    ...
    ImageData createImageData(FilterContext fctx, Rectangle bounds) {
        Filterable ret;
        try {
            // Allocate the cache texture at DEVICE resolution on HiDPI (Skia
            // non-scaleHint path); bounds stay LOGICAL for positioning. The cache
            // Graphics' pixel scale (set in renderNodeToCache) makes the node
            // render device-res into this larger texture, so it composites crisp.
            int tw = (cachePixelScaleX != 1.0f)
                ? (int) Math.ceil(bounds.width  * cachePixelScaleX) : bounds.width;
            int th = (cachePixelScaleY != 1.0f)
                ? (int) Math.ceil(bounds.height * cachePixelScaleY) : bounds.height;
            ret = Effect.getCompatibleImage(fctx, tw, th);
    ...
    void renderNodeToCache(ImageData cacheData,
        ...
        if (image != null) {
            ...
            // skia-fx HiDPI: render the node device-res into the (device-sized)
            // cache texture. We BAKE the pixel scale into the cache transform
            // (rather than setting it as a separate Graphics pixel-scale factor)
            // so the whole subtree renders with the scale in the CTM — the same
            // convention PrEffectHelper uses for effect children. Mixing the two
            // conventions (scale-separate here, scale-in-transform for nested
            // effect/clip children) diverged at the boundary and rendered the
            // region background at the wrong extent (content filled ~1/scale of
            // the cache after a monitor move). No-op (1.0) off the HiDPI
            // non-scaleHint path. Crisp, full cached regions on 150%/175%.
            TEMP_CACHEFILTER_TRANSFORM.setToIdentity();
            if (cachePixelScaleX != 1.0f || cachePixelScaleY != 1.0f) {
                TEMP_CACHEFILTER_TRANSFORM.scale(cachePixelScaleX, cachePixelScaleY);
            }
            ...
            if (dirtyBounds != null) {
                ...
                // The cache Graphics carries pixel-scale 1 (the device scale is
                // baked into TEMP_CACHEFILTER_TRANSFORM above, not set as a
                // Graphics pixel-scale factor), so SkiaGraphics.syncBeforeDraw
                // will NOT auto-scale this scissor. dirtyBounds is LOGICAL but
                // the cache content is DEVICE-res, so scale the scissor to device
                // here — otherwise the scroll repaint is clipped to 1/scale of
                // the dirty strip and stale/garbage pixels are left behind on a
                // 150%/175% monitor. No-op at 100%/scaleHint. (bugs.md NEW-H1)
                if (cachePixelScaleX != 1.0f || cachePixelScaleY != 1.0f) {
                    int cx = (int) Math.floor(dirtyBounds.x * cachePixelScaleX);
                    int cy = (int) Math.floor(dirtyBounds.y * cachePixelScaleY);
                    int cr = (int) Math.ceil((dirtyBounds.x + dirtyBounds.width)  * cachePixelScaleX);
                    int cb = (int) Math.ceil((dirtyBounds.y + dirtyBounds.height) * cachePixelScaleY);
                    TEMP_DIRTY_DEVICE.setBounds(cx, cy, cr - cx, cb - cy);
                    g.setClipRect(TEMP_DIRTY_DEVICE);
                } else {
    ...
    void moveCacheBy(ImageData cachedImageData, double xDelta, double yDelta) {
        ...
        // The cache texture is DEVICE-res on the Skia HiDPI non-scaleHint path
        // (createImageData sizes it ceil(bounds×cachePixelScale)), but xDelta/
        // yDelta and r are LOGICAL. Do the scroll-copy in DEVICE pixels: scale
        // the deltas AND the copied content extent by cachePixelScale. At 100%
        // / scaleHint (cachePixelScale==1) this is byte-identical to the previous
        // logical math (cw==r.width, dxDev==xDelta). (bugs.md NEW-H1)
        final double sx = cachePixelScaleX;
        final double sy = cachePixelScaleY;
        final int dxDev = (int) Math.round(xDelta * sx);
        final int dyDev = (int) Math.round(yDelta * sy);
        final int cw = (sx != 1.0) ? (int) Math.ceil(r.width  * sx) : r.width;
        final int ch = (sy != 1.0) ? (int) Math.ceil(r.height * sy) : r.height;
        int x = Math.max(0, -dxDev);
        int y = Math.max(0, -dyDev);
        int destX = Math.max(0, dxDev);
        int destY = Math.max(0, dyDev);
        int w = cw - Math.abs(dxDev);
        int h = ch - Math.abs(dyDev);
