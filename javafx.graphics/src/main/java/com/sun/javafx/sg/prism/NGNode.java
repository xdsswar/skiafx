...
public abstract class NGNode {
    ...
    // RESIZE INVARIANT #6 — effects are gated off during drag-resize.
    // ===============================================================
    // `effectsSupported` was originally a static-final boolean captured
    // at class init. Stays true while the pipeline reports effect
    // support (Skia: always), but flips to false during the
    // QuantumToolkit live-repaint window so the WHOLE effect dispatch
    // (renderEffect → EffectFilter.render → PrEffectHelper.render →
    //  fresh PrDrawable + saveLayer + filter chain + restore) is
    // skipped per node per frame while the user is dragging the
    // window edge. The peer-level `shouldBypassForDrag()` gate only
    // shortcuts the filter step itself; the source-content capture
    // upstream of the peer still allocates and copies. Gating at the
    // dispatch site here removes the full per-frame cost so the drag
    // is smooth, and shadows reappear the moment the drag stops.
    //
    // The cost of the dynamic check is one `System.nanoTime()` per
    // effect-bearing-node-per-frame. NGNode caches the pipeline ref
    // and the static "is effects pipeline supported at all" value, so
    // the per-call work is one volatile read + one long subtract +
    // one compare.
    private final static boolean effectsSupportedBase =
        (pipeline != null && pipeline.isEffectSupported());

    // Opt-in (default OFF): gate scene-graph effects off during the drag-resize
    // live-repaint window. This was a skia-fx drag-smoothness optimization, but
    // it diverges from stock JavaFX (which renders effects every frame) and left
    // effects stuck "plain" after a maximize/restore because nothing reliably
    // repaints once the gate clears. Default OFF restores stock parity: effects
    // always render. Set -Dskia.effects.gateOnResize=true to re-enable the
    // optimization on weak hardware. (isInLiveRepaint() is still used by
    // QuantumToolkit.postPulse for pulse-suppression — that is unaffected.)
    private static final boolean GATE_EFFECTS_ON_RESIZE =
        Boolean.getBoolean("skia.effects.gateOnResize");

    /** True iff effects are supported by the pipeline (and, only when the
     *  opt-in resize gate is enabled, we're not mid drag-resize). */
    private static boolean effectsSupported() {
        return effectsSupportedBase
            && (!GATE_EFFECTS_ON_RESIZE || !QuantumToolkit.isInLiveRepaint());
    }
...
//        }
    ...
    /**
     * Global monotonic counter — bumps on every dirty-tree event in any
     * NGNode anywhere in the scene graph. Used by the Skia
     * PresentingPainter as a change-detection signal: when this value
     * hasn't moved since the painter's previous successful present,
     * the scene is visually unchanged and the painter can skip both
     * paint and SwapBuffers for the current pulse. Bumped from
     * {@link #markTreeDirty()} — the universal entry point that
     * markDirty and markDirtyByTranslation both route through.
     */
    public static final AtomicLong SCENE_DIRTY_VERSION = new AtomicLong();
    ...
    protected final void markTreeDirty() {
        // Bump SCENE_DIRTY_VERSION first so PresentingPainter's
        // change-detection observes the latest dirty event even when
        // the actual mark-tree walk below short-circuits.
        SCENE_DIRTY_VERSION.incrementAndGet();
        NGNode p = parent;
...
//        return (union ^ (union << 31)) < 0;
    ...
    private void renderOpacity(Graphics g) {
        ...
        // the transform (curXform is LOGICAL). Sizing the offscreen from r
        // (logical) would render the subtree at logical resolution and upscale
        // it on composite -> blur on 150%/175% monitors. Allocate the offscreen
        // at DEVICE resolution and set the pixel scale on its Graphics so the
        // subtree (incl. nested effects) renders crisp; the composite-back is
        // unchanged (the device-res texture maps 1:1 once the window Graphics
        // re-applies the scale). No-op under Prism / at 100%.
        float psx = (g instanceof SkiaGraphics) ? g.getPixelScaleFactorX() : 1f;
        float psy = (g instanceof SkiaGraphics) ? g.getPixelScaleFactorY() : 1f;
        boolean hidpi = (psx != 1f || psy != 1f);
        int iw = hidpi ? (int) Math.ceil(r.width  * psx) : r.width;
        int ih = hidpi ? (int) Math.ceil(r.height * psy) : r.height;
        PrDrawable img = (PrDrawable)
            Effect.getCompatibleImage(fctx, iw, ih);
        if (img == null) {
            return;
        }
        Graphics gImg = img.createGraphics();
        if (hidpi) {
            gImg.setPixelScaleFactors(psx, psy);
        }
