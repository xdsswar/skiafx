...
public abstract class NGShape3D extends NGNode {
    ...
    private void setupLights(Graphics g) {
        ...
        if (noLights(lights)) {
            ...
            for (NGLightBase lightBase : lights) {
                ...
                */
                // Cap positional lights at the 8 the native shader supports.
                // Ambient (handled above) keeps accumulating past the cap; only
                // the slot-consuming lights stop, so the 9th+ light is dropped
                // here rather than relying on the native index guard (and without
                // the trailing reset loop being skipped by an over-incremented
                // lightIndex). Selection is still first-8-in-array-order — see the
                // relevance-sorting TODO above.
                else if (lightIndex < 8) {
                    if (lightBase instanceof NGSpotLight light) {
        ...
        // skia-fx: the bgfx pipeline supports up to MAX_LIGHTS (8) lights — see
        // openjfx_skia3d_bridge.cpp (stock Prism was limited to 3). Reset the rest.
        while (lightIndex < 8) { // MAX_LIGHTS — keep in sync with the native side
    ...
    private void addDirectionalLight(NGDirectionalLight light, int lightIndex, float r, float g, float b) {
        ...
        meshView.setLight(lightIndex,
                ...
                // Range is unused for a directional light (the shader gates the
                // range test on the positional flag), but it is still uploaded
                // verbatim into a GPU constant buffer — pass a finite sentinel
                // rather than POSITIVE_INFINITY so no non-finite value ever enters
                // a uniform (cleanliness / driver portability).
                Float.MAX_VALUE,         // range is irrelevant (directional)
                (float) normDir.getX(), (float) normDir.getY(), (float) normDir.getZ(),
    ...
    protected void renderContent(Graphics g) {
        if (!Platform.isSupported(ConditionalFeature.SCENE3D) ||
             material == null || g instanceof PrinterGraphics)
        {
            return;
        }
        renderMeshView(g);
