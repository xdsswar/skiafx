...
public class NGRegion extends NGGroup {
    ...
    private void renderAsShape(Graphics g) {
        if (!background.isEmpty()) {
            ...
            Rectangle rect = null;
            // skia-fx Phase E: same crash class as renderBackgroundRectangle
            // — the cached-RTT drawTexture(SkiaRTTexture) blit at the
            // "g.drawTexture(cached, ...)" call below routes through
            // surface_draw_surface and corrupts the shared GrDirectContext
            // when src and dst live in it (always, in our setup). Skip the
            // RTT cache for Skia surfaces; renderBackgroundShape draws
            // straight to the live canvas.
            final boolean skiaBackend = SkiaSurfaceAccess.handleOf(g) != 0L;
            ...
            if (!skiaBackend && cacheMode != 0 && g.getTransformNoClone().isTranslateOrIdentity() && !(g instanceof PrinterGraphics)) {
    ...
    private void renderBackgroundRectangle(Graphics g) {
        ...
        //
        // skia-fx Phase E (plan twinkly-splashing-dove): the RTT-based
        // cache decision below allocates a cache RTT, renders into it,
        // then composites back via drawTexture3SliceH/V which routes to
        // SkiaGraphics.drawTexture(SkiaRTTexture) → surface_draw_surface.
        // That blit corrupts the shared GrDirectContext (see memory note
        // webview-open-followups #12 — both the form-chrome and scrollbar
        // crash stacks land here). For Skia-backed Graphics we bypass the
        // RTT cache entirely and render fills straight to the live
        // canvas; SkCanvas batches the ops, and no second surface is
        // involved. Modest extra per-frame cost (re-issuing the fill
        // commands per region per frame) — typical scene has <50 visible
        // Regions, so the cost is negligible vs. eliminating the crash.
        // A follow-up (Phase F) can layer SkPicture caching on top to
        // restore the work-amortization that the RTT cache provided.
        final boolean skiaBackend = SkiaSurfaceAccess.handleOf(g) != 0L;
        final boolean cache =
                !skiaBackend &&
