            ...
            if (!error && !exitCalled.get()) {
                ...
                PlatformImpl.runAndWait(() -> {
                    try {
                        ...
                        // The stage is built via Application#createPrimaryStage()
                        // so an app can supply a custom Stage subclass as its
                        // primary window (skia-fx; see docs/CUSTOM_PRIMARY_STAGE.md).
