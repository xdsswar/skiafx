...
public class Utils {
    ...
    /**
     * Historically loaded the {@code prism_common} native library for the
     * Swing interop path. skia-fx is 100% Prism-free — the Prism backend
     * natives (including {@code prism_common}) were removed and Skia replaces
     * them. The Swing interop modules declare no native methods of their own
     * (SwingNode renders through an {@code int[]} ARGB buffer + AWT/glass), so
     * nothing here needs a native library. Kept as a no-op so existing
     * call sites (e.g. {@code SwingNodeInteropN}) compile and run unchanged.
     */
    public static synchronized void loadNativeSwingLibrary() {
        // No-op: no prism_common (zero-Prism); Swing interop needs no native lib.
    }
