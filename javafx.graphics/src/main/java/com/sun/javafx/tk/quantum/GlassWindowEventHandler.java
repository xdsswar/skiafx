    ...
    public Void get() {
        ...
        switch (type) {
            ...
            case WindowEvent.RESCALE: {
                ...
                // skia-fx: the Scene's LOGICAL size is recomputed (as deviceSize /
                // platformScale) only in the RESIZE handler above. On a monitor move
                // to a different DPI, the new device size (WM_SIZE) and the new scale
                // (notifyScaleChanged) arrive via independent native paths; at large
                // scale ratios (e.g. 300%->100%) the final flip can land here as a
                // scale-only RESCALE, or a RESIZE can run while platformScale is still
                // stale, leaving the Scene logical size at deviceW/oldScale (≈1/3) —
                // so the root lays out small and paints into the top-left corner of a
                // correctly-sized surface (rest = white). Recompute the Scene size here
                // too, reading size and scale TOGETHER so they can't disagree.
                // changedSize is idempotent (no-ops on equality), so this is free when
                // RESIZE already set the right size and never double-applies.
                float pScaleX = window.getPlatformScaleX();
                float pScaleY = window.getPlatformScaleY();
