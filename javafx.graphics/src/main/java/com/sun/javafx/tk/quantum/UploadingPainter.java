...
final class UploadingPainter extends ViewPainter {
    ...
    @Override public void run() {
        ...
        try {
            ...
            // transform — SkiaGraphics applies it per draw (and to the clip) using
            // the factors paintImpl sets via g.setPixelScaleFactors. Baking it into
            // the CTM here too (the Prism convention) double-scales every draw
            // (transform ×scale × per-draw ×scale = scale²), overflowing the
            // device-sized RTTexture by `scale` so upload-tier windows (popups,
            // ComboBox/Menu/ColorPicker/DatePicker dropdowns, transparent stages)
            // rendered at ~1/scale too large — only part fit, with offset hover and
            // cleared "holes". PresentingPainter (direct-present main window) never
            // calls g.scale, which is why it was unaffected. Keep the bake for the
            // Prism pipeline, which folds pixel scale into the transform.
            if (!(g instanceof SkiaGraphics)) {
                g.scale(scalex, scaley);
            } else {
                // skia-fx: force a full repaint on the upload tier. The dirty-
                // region partial repaint reuses the persisted RTTexture and only
                // redraws the changed sub-rect; on Skia that left hover repaints
                // distorted/ghosted (stale pixels under the transparent popup
                // shadow were not cleared). Upload-tier windows (popups, dropdowns)
                // are small, so repainting the whole RTTexture each pulse is cheap
                // and renders cleanly every frame. (The full-repaint open frame was
                // already correct.)
                freshBackBuffer = true;
            }
            paintImpl(g);
