...
public final class QuantumToolkit extends Toolkit {
    ...
    // Timestamp of the last PaintCollector.liveRepaintRenderJob call.
    // While a drag-resize is active the master pulse timer would
    // otherwise interleave 1ms-cadence pulses with the synchronous
    // WM_SIZE-driven repaints — that interleave paints stale frames
    // between OS resize ticks and was the primary cause of the visible
    // ScrollPane shrink/expand shimmer on non-maximized resize. We gate
    // postPulse on this timestamp instead of plumbing WM_ENTERSIZEMOVE /
    // WM_EXITSIZEMOVE flags through Glass: a drag is "active" iff a
    // live-repaint fired within the last DRAG_GATE_NANOS.
    private static volatile long lastLiveRepaintNanos = 0L;
    private static final long DRAG_GATE_NANOS = 100_000_000L; // 100 ms

    /**
     * Called from {@link PaintCollector#liveRepaintRenderJob} on every
     * WM_SIZE-driven synchronous pulse so {@link #postPulse()} knows to
     * suppress concurrent timer pulses for the next
     * {@value #DRAG_GATE_NANOS}-nanosecond window.
     */
    static void noteLiveRepaint() {
        lastLiveRepaintNanos = System.nanoTime();
    }

    /**
     * True if a {@code liveRepaintRenderJob} fired within the last
     * {@value #DRAG_GATE_NANOS} ns — i.e. the user is mid-drag-resize
     * and the FX thread is in a tight WM_SIZE / synchronous-pulse
     * loop. Lets the Skia effect engine (and any other expensive
     * per-frame consumer) short-circuit during that window so it
     * doesn't pay the per-frame GPU cost while the user is dragging.
     */
    public static boolean isInLiveRepaint() {
        return (System.nanoTime() - lastLiveRepaintNanos) < DRAG_GATE_NANOS;
    }

    /**
     * skia-fx: drive a JavaFX pulse from a native WM_TIMER handler
     * that runs inside a Win32 modal loop (title-bar drag,
     * caption-button hold on a stock decorated window). The OS
     * runs its own nested message pump inside {@code DefWindowProc}
     * while the user holds the mouse; the normal pulse path
     * ({@code postPulse → Application.invokeLater}) posts a
     * {@code WM_DO_ACTION_LATER} that the caption-button modal pump
     * does <em>not</em> dispatch — animations and video freeze until
     * release.
     *
     * <p>The native side bracket calls {@code SetTimer} on the
     * window; WM_TIMER <em>is</em> dispatched inside the modal pump,
     * which is verified by the {@code OPENJFX_MODAL_PULSE_DIAG}
     * counter. The handler then enters this method on the FX thread
     * (Glass's main thread) and runs the pulse inline via
     * {@link #pulseFromQueue()} — bypassing the
     * {@code invokeLater} queue entirely so it doesn't matter
     * whether the modal pump dispatches posted messages.</p>
     *
     * <p>The gate flags from {@link #postPulse()} are honoured: we
     * skip when {@code toolkitRunning} is false, when nothing wants
     * a pulse, or when {@code dragActive} is true (live-repaint is
     * running on the FX thread; an inline pulse would double-paint).</p>
     */
    public static void firePulseFromNativeTimer() {
        Toolkit tk = Toolkit.getToolkit();
        if (!(tk instanceof QuantumToolkit qt)) return;
        if (!qt.toolkitRunning.get()) return;
        if (!qt.animationRunning.get() && !qt.nextPulseRequested.get()) return;
        if ((System.nanoTime() - lastLiveRepaintNanos) < DRAG_GATE_NANOS) return;
        if (qt.pulseRunning.getAndSet(true)) return;
        try {
            qt.pulseFromQueue();
        } catch (Throwable t) {
            qt.pulseRunning.set(false);
            throw t;
        }
        // pulseFromQueue's finally clears pulseRunning via endPulseRunning,
        // so no explicit reset on the success path.
    }

    void postPulse() {
        // See lastLiveRepaintNanos.
        final boolean dragActive =
            (System.nanoTime() - lastLiveRepaintNanos) < DRAG_GATE_NANOS;
    ...
    public void dispose() {
        if (toolkitRunning.compareAndSet(true, false)) {
            ...
            // Final drain of deferred native/GPU frees. These are normally drained
            // once per pulse in PresentingPainter, but pulses have now stopped — so
            // anything a Cleaner enqueued after the last pulse (e.g. a GC'd
            // SkiaMediaTexture's GPU SkImage + D3D interop) would otherwise leak at
            // exit. Safe here: the render thread is confirmed stopped (no
            // concurrency), each queued action is idempotent, and the queue is
            // empty in the common case. (bugs.md M17)
