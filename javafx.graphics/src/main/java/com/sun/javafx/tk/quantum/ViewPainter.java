...
abstract class ViewPainter implements Runnable {
    ...
    protected void paintImpl(final Graphics backBufferGraphics) {
        ...
        if (width <= 0 || height <= 0 || backBufferGraphics == null) {
            if (Boolean.getBoolean("skia.resize.diag")) {
                System.err.println("[skia.paint] BAIL (no content drawn) w=" + width
                    + " h=" + height + " g=" + (backBufferGraphics != null)
                    + " scene=" + sceneState.getScene().getClass().getSimpleName());
            }
            root.renderForcedContent(backBufferGraphics);
            return;
        }
        if (Boolean.getBoolean("skia.resize.diag")) {
            System.err.println("[skia.paint] PAINT w=" + width + " h=" + height
                + " scene=" + sceneState.getScene().getClass().getSimpleName());
        }
        ...
        if (dirtyRegionSize > 0) {
            ...
            for (int i = 0; i < dirtyRegionSize; ++i) {
                ...
                if (dirtyRegion.getWidth() > 0 && dirtyRegion.getHeight() > 0) {
                    ...
                    // Graphics transform and applies it (to BOTH the matrix and the clip
                    // rect) per-draw in SkiaGraphics.syncBeforeDraw. So the clip rect it
                    // expects is LOGICAL — pre-multiplying the dirty region by pixelScale
                    // here would double-scale it (×scale²), which mis-clipped every partial
                    // repaint on 150%/175% monitors: hover dirty-regions cleared/clipped at
                    // the wrong, larger, offset area (popup menu hover highlights overflowed,
                    // ColorPicker swatches showed white holes). Stock Prism bakes pixelScale
                    // into the transform, so there the clip must be DEVICE — keep that path.
                    final float clipSX = (g instanceof SkiaGraphics) ? 1f : pixelScaleX;
                    final float clipSY = (g instanceof SkiaGraphics) ? 1f : pixelScaleY;
                    int x0, y0;
                    dirtyRect.x = x0 = (int) Math.floor(dirtyRegion.getMinX() * clipSX);
                    dirtyRect.y = y0 = (int) Math.floor(dirtyRegion.getMinY() * clipSY);
                    dirtyRect.width  = (int) Math.ceil (dirtyRegion.getMaxX() * clipSX) - x0;
                    dirtyRect.height = (int) Math.ceil (dirtyRegion.getMaxY() * clipSY) - y0;
    ...
    protected boolean validateStageGraphics() {
        ...
        // skia-fx: paint-before-show. When the owning WindowStage
        // is in its pre-show paint phase (after super.setVisible
        // opens the render gate but before the native ShowWindow
        // fires), allow paint to the still-hidden HWND's swap
        // chain. The Present lands in DWM's redirection bitmap,
        // so the subsequent OS show animation reveals real scene
        // content instead of the default white. See
        // WindowStage.paintBeforeShow.
