...
final class PaintCollector implements CompletionListener {
    ...
    */
    ...
    @Override public void done(RenderJob job) {
        ...
        // Everything runs inside try/finally so the latch ALWAYS counts down,
        // even if an invariant check or a post-render callback below throws. The
        // latch is what waitForRenderingToComplete() blocks the FX thread on; a
        // throw that skipped the count-down (the bare throws at the two invalid-
        // job guards, or a callback failure) would strand that waiter forever and
        // deadlock the FX thread. Counting down on the throw path only unblocks a
        // stuck waiter — it never double-counts (single boundary). (bugs.md M7)
        try {
    ...
    // RESIZE INVARIANT #7 — No sync paint inside the WM_SIZE drag gate.
    // =================================================================
    // Windows fires WM_SIZE at ~80 Hz during a fast drag. On the GL
    // tier, FBO 0's back buffer reallocates *one frame behind* the
    // WM_SIZE (Windows defers the reallocation to the next
    // SwapBuffers — invariant #3), and the wgl spec offers no path
    // to force the reallocation earlier without an extra SwapBuffers
    // (which causes ghost trails + flicker — invariant #3 forbids).
    //
    // The chain of attempts in earlier iterations was each visually
    // worse:
    //  - sync paint per WM_SIZE — content rasterizes at logical size
    //    W_actual but lands in a back buffer at physical size W_old.
    //    DWM stretches W_old → W_actual → text/edges land at non-
    //    integer physical positions → blurry text and content that
    //    appears to "shrink faster than the ScrollPane".
    //  - throttled sync paint (33 ms) — back buffer holds a static
    //    size across multiple WM_SIZEs while DWM keeps stretching to
    //    the moving window → stretch ratio drifts within the 33 ms
    //    gap → "shrink/expand in a loop".
    //
    // We can't fix the GL back-buffer lag without violating invariant
    // #3, so we stop trying. Instead: don't sync-paint during the
    // drag at all. DWM rubber-band-stretches the last clean frame
    // (title bar + content + scrollpane all together) uniformly with
    // the window edge — the same visual model Win32 / stock JFX have
    // always used on Windows. The user reads it as smooth resize.
    //
    // Settle pulse: the master pulse timer is suppressed during the
    // DRAG_GATE_NANOS window (100 ms). When the user releases the
    // mouse, the timer resumes and the next pulse paints the scene
    // at its final size. nextPulseRequested needs to be set; we set
    // it here on every WM_SIZE so a fresh pulse is queued regardless
    // of whether the scene's dirty bits independently triggered one.
    //
    // Note we still call noteLiveRepaint() so:
    //  - The NGNode effect-dispatch gate (RESIZE_INVARIANT #6) flips
    //    off during drag — Modena scrollbar shadows are skipped.
    //  - QuantumToolkit.postPulse suppresses timer pulses during the
    //    drag (no interleave with the master pulse).
    private static final boolean RESIZE_DIAG =
        Boolean.getBoolean("skia.resize.diag");
    private static volatile long lastResizeDiagNanos = 0L;

    /**
     * Run a full pulse and repaint before returning. Called from
     * Glass on every WM_SIZE-driven view-size change.
     *
     * <p>Painting per WM_SIZE is the right move when the active
     * present tier handles the resize cleanly (D3D12: synchronous
     * {@code ResizeBuffers}, no DWM stretch). On GL the back-buffer
     * lag (invariant #3) means this paint sometimes lands in a
     * 1-WM_SIZE-old buffer that DWM then stretches, producing the
     * "diffusion" artifact — that is the documented architectural
     * cost of running on tier 1 GL, and the reason we default to
     * tier 0 D3D12 on capable hardware.</p>
     */
    final void liveRepaintRenderJob(final ViewScene scene) {
         // Mark the live-repaint window so:
         //  - NGNode.effectsSupported() flips off (invariant #6) — effects are
         //    skipped per node per frame for drag smoothness,
         //  - QuantumToolkit.postPulse suppresses timer pulses for the next
         //    100 ms.
         // noteLiveRepaint() still gates QuantumToolkit.postPulse pulse
         // suppression during the drag (the ScrollPane shimmer fix). It no
         // longer gates effects: NGNode.effectsSupported() ignores it by
         // default (see skia.effects.gateOnResize), so each WM_SIZE frame below
         // paints WITH effects — matching stock JavaFX and leaving no stuck
         // "plain effects" state once the drag ends.
         QuantumToolkit.noteLiveRepaint();

         final QuantumToolkit quantum = (QuantumToolkit)QuantumToolkit.getToolkit();
         quantum.requestNextPulse();

         if (RESIZE_DIAG) logResizeEvent(scene, "paint");
    ...
    /** -Dskia.resize.diag=true: one line per liveRepaintRenderJob.
     *  Timing only — the painter logs its own width/height on first
     *  paint so cross-referencing is straightforward. */
    private static void logResizeEvent(ViewScene scene, String action) {
        long now = System.nanoTime();
        long deltaMs = (lastResizeDiagNanos == 0L)
            ? 0L : (now - lastResizeDiagNanos) / 1_000_000L;
        lastResizeDiagNanos = now;
        ViewPainter vp = scene == null ? null : scene.getPainter();
        int w = -1, h = -1;
        if (vp != null) { w = vp.viewWidth; h = vp.viewHeight; }
        System.err.printf("[skia.resize] t=%d ms  delta=%d ms  size=%dx%d  action=%s%n",
            now / 1_000_000L, deltaMs, w, h, action);
    }
