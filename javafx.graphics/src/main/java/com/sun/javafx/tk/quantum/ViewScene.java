...
class ViewScene extends GlassScene {
    ...
    public void setStage(GlassStage stage) {
        ...
        if (stage != null) {
            ...
            boolean upload = wstage.needsUpdateWindow() || GraphicsPipeline.getPipeline().isUploading();
            // One-shot diagnostic: tells us which painter this stage
            // ended up using, and *why* if the slow path was picked.
            // Gated behind -Dskia.verbose=true now that the direct-present
            // path is the proven default.
            if (Boolean.getBoolean("skia.verbose")) {
                System.err.println("[skia.painter] stage=" + wstage
                    + " painter=" + (upload ? "UploadingPainter" : "PresentingPainter")
                    + " needsUpdateWindow=" + wstage.needsUpdateWindow()
                    + " isUploading=" + GraphicsPipeline.getPipeline().isUploading());
            }
            if (upload) {
